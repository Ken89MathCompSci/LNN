"""
GS-LNN: Gated + Spectral-Stable Liquid Neural Network — UKDALE specific splits
===============================================================================
Combines two orthogonal improvements over the base LNN:

  1. Residual gated liquid transport  (controls WHAT updates)
  2. Spectral-norm constraint on U    (controls HOW FAST dynamics grow)

─────────────────────────────────────────────────────────────────────────────
Base LNN Euler step
    dh/dt   = -h/τ + tanh(W_in x + U h)
    h_{t+1} = h_t + dt · dh/dt               ← uniform, unconstrained update

─────────────────────────────────────────────────────────────────────────────
Improvement 1 — Residual gated transport
    g_t     = σ(W_g h_t + b_g)               ← per-neuron gate ∈ (0,1)
    h_{t+1} = h_t + g_t ⊙ dt · dh/dt        ← gated update

    • g_t → 0 : hidden state frozen (persistent memory — good for fridge)
    • g_t → 1 : full Euler step (standard LNN — good for microwave spikes)
    • Phase-dependent gate: WD transitions open/close the gate per neuron
    • Controls ‖DF‖ (transport amplification) → bounds gradient flow

    Gate init: W_g ~ Xavier, b_g = 0  →  σ(0) = 0.5 at epoch 0, matching
    the base LNN at initialisation so training starts from a familiar basin.

─────────────────────────────────────────────────────────────────────────────
Improvement 2 — Spectral stability constraint
    After every optimizer step:
        σ_max = ‖U‖_2   (largest singular value)
        if σ_max > ρ_max:
            U ← U × (ρ_max / σ_max)          ← project onto spectral ball

    ρ_max = 0.99 keeps the system at marginal stability:
        ρ < 1 → stable, decaying memory
        ρ = 1 → marginal (echo-state networks)
        ρ > 1 → unstable (forbidden — appliances are dissipative)

    Physical justification: real appliances do not exhibit exponentially
    growing energy, so ρ(U) > 1 is physically incorrect.

─────────────────────────────────────────────────────────────────────────────
Why the two improvements are complementary
    The gate bounds per-step transport magnitude (∥g ⊙ dh∥ ≤ ∥dh∥).
    The spectral constraint bounds the recurrent Jacobian (‖∂h'/∂h‖ ≤ ρ).
    Together they constrain both the magnitude and the propagation of
    hidden-state changes — addressing gradient stability from two angles.
...
Diagnostics saved to JSON:
    spec_h — σ_max(U) per epoch (post-clip); verify constraint holds throughout

Usage:
    python test_gslnn_ukdale_specific_splits.py --appliance fridge
    python test_gslnn_ukdale_specific_splits.py --appliance microwave
    python test_gslnn_ukdale_specific_splits.py --appliance "dish washer"
    python test_gslnn_ukdale_specific_splits.py --appliance "washer dryer"
    python test_gslnn_ukdale_specific_splits.py --plot
"""
...
SPEC_RADIUS   = 0.99   # spectral-norm ceiling for U
...
SAVE_DIR = os.path.join('results', 'gslnn_ukdale')
COLOR    = '#D62728'   # red — distinct from basic (green), G-LNN (orange), S-LNN (purple)
...
class GatedSpectralLiquidTimeLayer(nn.Module):
    """
    Liquid Time-Constant layer with both residual gating and spectral stability.

    ODE:  dh/dt = -h/τ + tanh(W_in x + U h)
    Gate: g     = σ(W_g h + b_g)              ∈ (0,1)^H
    Step: h'    = clamp(h + g ⊙ dt · dh, -10, 10)

    After every optimizer step (external call, not in forward):
        σ = ‖U‖_2;  if σ > SPEC_RADIUS: U ← U × (SPEC_RADIUS / σ)
    ...
    Parameter count vs base LiquidTimeLayer
        Base  : W_in (H×1+H)  +  tau (H)  +  U (H×H)  +  fc (H×1+1)
        GS    : + gate_proj  (H×H + H)   ← only addition
    """
    ...
    def __init__(self, input_size: int, hidden_size: int, dt: float = 0.1):
        ...
        # Recurrent matrix as nn.Linear so .weight is accessible for spectral clipping
        self.U = nn.Linear(hidden_size, hidden_size, bias=False)
        nn.init.xavier_uniform_(self.U.weight)

        # Residual gate: maps current hidden state → per-neuron gate
        # Zero bias → σ(0) = 0.5 at init, matching half-step of base LNN
    ...
    def forward(self, x: torch.Tensor, hidden: torch.Tensor | None = None) -> torch.Tensor:
        ...
        dh   = -hidden / self.tau.unsqueeze(0) + f_t    # ODE derivative
        gate = torch.sigmoid(self.gate_proj(hidden))     # per-neuron gate ∈ (0,1)
...
class GatedSpectralLiquidNetworkModel(nn.Module):
    """
    Unrolls GatedSpectralLiquidTimeLayer over WIN timesteps → FC head.
...
def _spectral_norm(model: GatedSpectralLiquidNetworkModel) -> float:
    with torch.no_grad():
...
def train_appliance(appliance_name, splits, device, epochs, hidden_size):
    ...
    model = GatedSpectralLiquidNetworkModel(
    ...
    print(f"  Gate extra vs S-LNN: {gate_extra:,}  (W_g: {hidden_size}×{hidden_size} + b_g)")
    ...
    for epoch in range(epochs):
        ...
        for xb, yb in bar:
            ...
            # ── Spectral clipping: project U onto {‖U‖₂ ≤ SPEC_RADIUS} ──────
            with torch.no_grad():
...
def plot_training_curves(results, save_dir):
    ...
    fig.suptitle('Training Curves — GS-LNN (UKDALE)', fontsize=12)
    plt.tight_layout()
...
def plot_epoch_metrics(results, save_dir):
    ...
    fig.suptitle('Val Metrics per Epoch — GS-LNN (UKDALE)', fontsize=12)
    plt.tight_layout()
...
def plot_bar_chart(results, save_dir):
    ...
    fig.suptitle('Final Test Metrics — GS-LNN (UKDALE)', fontsize=12)
    plt.tight_layout()
...
def _save_json(app, r, hidden, epochs, save_dir):
    ...
    with open(path, 'w') as f:
        json.dump({
            'appliance':     app,
            'dataset':       'UKDALE',
            'architecture':  'GatedSpectralLiquidNetworkModel',
            'hidden_size':   hidden,
...
def main():
    parser = argparse.ArgumentParser(description='GS-LNN — UKDALE')
    ...
    print('Architecture: GatedSpectralLiquidTimeLayer → hidden → FC')
    ...
    if all_done:
        ...
        print('Run those, then: python test_gslnn_ukdale_specific_splits.py --plot')
