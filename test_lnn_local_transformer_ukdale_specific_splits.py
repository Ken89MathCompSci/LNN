"""
LNN-First Hybrid Transformer with Sliding Window Attention — UKDALE specific splits
====================================================================================
Architecture: LNN (physics frontend) → Local Transformer Encoder (banded attention)

The LiquidODECell is unrolled over every raw timestep first, building a
physics-aware hidden-state sequence.  The Transformer Encoder then applies
*banded* (sliding window) self-attention so each position t can only attend
to [t−ATTN_WINDOW, t+ATTN_WINDOW].  This prevents transient ON/OFF edges
from bleeding across distant timesteps while the LNN recurrent state still
carries global history.

  Input proj    : Linear(1 → H) + LayerNorm + GELU
  LNN frontend  : LiquidODECell unrolled over WIN timesteps  → (B, WIN, H)
  LNN dropout   : Dropout(0.1)
  Local Attn    : 2 × TransformerEncoderLayer with window k=ATTN_WINDOW
  Pool          : mean (fridge / dish washer / washer dryer)  |  max (microwave)
  FC head       : Linear(H → 1)
...
Model selection:
  Warmup phase  — track best val_loss  (MSE scale)
  BCE phase     — track best val F1    (avoids always-ON collapse on skewed days)

Usage (Colab / terminal):
    # one appliance at a time:
    !python test_lnn_local_transformer_ukdale_specific_splits.py --appliance fridge
    !python test_lnn_local_transformer_ukdale_specific_splits.py --appliance microwave
    !python test_lnn_local_transformer_ukdale_specific_splits.py --appliance "dish washer"
    !python test_lnn_local_transformer_ukdale_specific_splits.py --appliance "washer dryer"

    # regenerate plots after all four are done:
    !python test_lnn_local_transformer_ukdale_specific_splits.py --plot

    # optional overrides:
    !python test_lnn_local_transformer_ukdale_specific_splits.py --hidden 128 --epochs 60
"""
...
from models import LNNLocalTransformerModel
...
WIN           = 100
STRIDE        = 5
ATTN_WINDOW   = 20          # each position attends to [t-k, t+k]

WARMUP_EPOCHS = 15          # epochs trained with MSE only before BCE is added
...
SAVE_DIR = os.path.join('results', 'lnn_local_transformer_ukdale')
COLOR    = '#9467BD'
...
def load_data():
    ...
    d = splits['train']
    print(f"  Train: {d.index.min()} → {d.index.max()}")
    d = splits['val']
    print(f"  Val  : {d.index.min()} → {d.index.max()}")
    d = splits['test']
    print(f"  Test : {d.index.min()} → {d.index.max()}")
...
def train_appliance(appliance_name, splits, device, epochs, hidden_size):
    ...
    pool       = POOL_MODE[appliance_name]

    print(f"\n{'='*60}")
    print(f"  {appliance_name}  |  hidden={hidden_size}  pool={pool}")
    print(f"  λ_bce={bce_lam}  α_bce={bce_alpha}  warmup={WARMUP_EPOCHS}  window={ATTN_WINDOW}")
    print(f"{'='*60}")
    ...
    model = LNNLocalTransformerModel(
        input_size=1, hidden_size=hidden_size, output_size=1,
        dt=0.1, num_encoder_layers=2, num_heads=4,
        dropout=0.1, pool=pool,
        seq_len=WIN, attn_window=ATTN_WINDOW,
    ).to(device)
    ...
    for epoch in range(epochs):
        ...
        # Reset tracker at warmup→BCE boundary; switch criterion to val F1
        if epoch == WARMUP_EPOCHS:
            best_val    = float('inf')
            best_val_f1 = -1.0
            no_improve  = 0
...
def plot_training_curves(results, save_dir):
    ...
    fig.suptitle('Training Curves — LNN-Local Transformer (UKDALE)', fontsize=12)
    plt.tight_layout()
...
def plot_epoch_metrics(results, save_dir):
    ...
    fig.suptitle('Val Metrics per Epoch — LNN-Local Transformer (UKDALE)', fontsize=12)
    plt.tight_layout()
...
def plot_bar_chart(results, save_dir):
    ...
    fig.suptitle('Final Test Metrics — LNN-Local Transformer (UKDALE)', fontsize=12)
    plt.tight_layout()
...
def print_table(results):
    ...
    for metric, label in [('f1', 'F1'), ('precision', 'Precision'),
        ...
        print(f"\n{'='*70}")
        print(f"  {label} — LNN-Local Transformer (UKDALE)")
        print(f"{'='*70}")
...
def _save_json(app, r, hidden, epochs, save_dir):
    ...
    with open(path, 'w') as f:
        json.dump({
            'appliance':    app,
            'dataset':      'UKDALE',
            'architecture': 'LNN-First → Local Transformer',
            'hidden_size':  hidden,
...
def main():
    parser = argparse.ArgumentParser(
        description='LNN-First Local Transformer — UKDALE')
    ...
    print(f'Architecture: Input Proj → LNN → Local Transformer (window={ATTN_WINDOW}) → Pool → FC')
    ...
    if all_done:
        ...
        print(f'Run those, then: python test_lnn_local_transformer_ukdale_specific_splits.py --plot')
