...
def train_on_appliance(data_dict, appliance_name, save_dir,
    ...
    plt.savefig(os.path.join(save_dir,
        f"multiscale_lnn_redd_{appliance_name.replace(' ', '_')}_metrics.png"),
    ...
    with open(os.path.join(save_dir,
            f'multiscale_lnn_redd_{appliance_name.replace(" ", "_")}_history.json'),
...
def run_all(hidden_size=64, dt_fast=DT_FAST, dt_med=DT_MED, dt_slow=DT_SLOW):
    ...
    base_save_dir = f"models/multiscale_lnn_redd_test_{timestamp}"
    ...
    print(f"\nMulti-Scale-LNN REDD testing completed.")
