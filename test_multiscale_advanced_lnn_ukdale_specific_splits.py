"""
Multi-Scale Advanced LNN — parallel AdvancedLiquidTimeLayer heads with different dt timescales.

Enhancement over MultiScaleLiquidNetworkModel:
  Each head is upgraded from a basic fixed-tau cell to an AdvancedLiquidTimeLayer:
    - Adaptive tau  : tau = softplus(tau_base) * sigmoid(W_tau · x_t)  — input-dependent
    - Input-dependent gate : gate = sigmoid(W_gate · [x_t, h_{t-1}])
    - ODE: dh = (-h/tau + gate * tanh(W_in·x + W_rec·h)) * dt

  Three parallel heads with different fixed dt biases:
    h_fast = AdvancedHead(dt=0.1)(x)   # rapid bursts   (microwave)
    h_med  = AdvancedHead(dt=0.5)(x)   # medium cycles  (dish washer)
    h_slow = AdvancedHead(dt=2.0)(x)   # slow dynamics  (washing machine)
    h_combined = concat([h_fast, h_med, h_slow])   # 192-dim
    h_fused    = Linear(192->64) + ReLU + LayerNorm
    output     = Linear(64->1)

Each head is biased toward its timescale via dt but can adapt within that timescale
via its own tau and gate — unlike the basic multi-scale variant where tau is fixed.
"""
...
# ---------------------------------------------------------------------------
# Advanced LNN head  (adaptive tau + input-dependent gate, single layer)
# ---------------------------------------------------------------------------

class AdvancedLiquidHead(nn.Module):
    """
    Single AdvancedLiquidTimeLayer cell with a fixed dt bias.
    Adaptive tau and input-dependent gate give each head full expressive power
    within its designated timescale.
    """
    def __init__(self, input_size, hidden_size, dt):
        super(AdvancedLiquidHead, self).__init__()
    ...
    def forward(self, x):
        ...
        for t in range(seq_len):
            ...
            tau_mod  = torch.sigmoid(self.tau_mod(x_t))                 # (batch, hidden)
            tau      = (tau_base * tau_mod).clamp(min=self.dt)          # (batch, hidden)

            gate = torch.sigmoid(self.gate(torch.cat([x_t, h], dim=1))) # (batch, hidden)
...
# ---------------------------------------------------------------------------
# Multi-Scale Advanced LNN
# ---------------------------------------------------------------------------

class MultiScaleAdvancedLiquidNetworkModel(nn.Module):
    """
    Three parallel AdvancedLiquidHead instances with different dt values, fused before output.

    Fast head  (dt=0.1): reacts to rapid power spikes, gate suppresses slow baseline
    Medium head(dt=0.5): tracks medium-length cycles with adaptive tau
    Slow head  (dt=2.0): captures long-duration states, gate filters transient noise
    ...
    def __init__(self, input_size, hidden_size, output_size,
        ...
        super(MultiScaleAdvancedLiquidNetworkModel, self).__init__()
        self.hidden_size = hidden_size

        self.head_fast = AdvancedLiquidHead(input_size, hidden_size, dt=dt_fast)
        self.head_med  = AdvancedLiquidHead(input_size, hidden_size, dt=dt_med)
        self.head_slow = AdvancedLiquidHead(input_size, hidden_size, dt=dt_slow)
    ...
    def forward(self, x):
        h_fast = self.head_fast(x)
        h_med  = self.head_med(x)
        h_slow = self.head_slow(x)

        h_combined = torch.cat([h_fast, h_med, h_slow], dim=1)
...
def train_on_appliance(data_dict, appliance_name, save_dir,
    ...
    model = MultiScaleAdvancedLiquidNetworkModel(
    ...
    print(f"Starting Multi-Scale-Advanced-LNN training for {appliance_name}...")
    ...
    plt.savefig(os.path.join(save_dir,
        f"multiscale_advanced_lnn_ukdale_{appliance_name.replace(' ', '_')}_metrics.png"),
    ...
    config = {
        ...
        'dataset': 'UKDALE',
        'model': 'MultiScaleAdvancedLiquidNetworkModel',
        'enhancement': 'parallel AdvancedLiquidHead (adaptive tau + gate) with fast/med/slow dt',
        'loss': 'MSE',
        'window_size': WIN,
        'model_params': {
    ...
    with open(os.path.join(save_dir,
            f'multiscale_advanced_lnn_ukdale_{appliance_name.replace(" ", "_")}_history.json'),
...
def run_all(hidden_size=64, dt_fast=DT_FAST, dt_med=DT_MED, dt_slow=DT_SLOW):
    ...
    base_save_dir = f"models/multiscale_advanced_lnn_ukdale_test_{timestamp}"

    all_results = {}

    for appliance_name in APPLIANCES:
        print(f"\n{'='*60}")
        print(f"Testing Multi-Scale-Advanced-LNN on {appliance_name} (UKDALE)")
        print(f"  dt_fast={dt_fast}  dt_med={dt_med}  dt_slow={dt_slow}")
        print(f"{'='*60}\n")
    ...
    print(f"\nMulti-Scale-Advanced-LNN UKDALE testing completed.")
