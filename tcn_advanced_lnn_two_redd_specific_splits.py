...
def train_on_appliance(data_dict, appliance_name, window_size=100,
                       ...
                       save_dir='models/tcn_adv_lnn2_redd'):
    ...
    print(f"Starting TCN-AdvLNN2 training for {appliance_name}...")
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"tcn_adv_lnn2_{appliance_name.replace(' ', '_')}_best.pth")
            save_model(model,
    ...
    plt.savefig(os.path.join(save_dir,
        f"tcn_adv_lnn2_{appliance_name.replace(' ', '_')}_metrics.png"),
    ...
    config = {
        ...
        'model_params': {
            'input_size': input_size, 'output_size': output_size,
            'num_channels': num_channels, 'kernel_size': kernel_size,
            'dropout': dropout, 'num_layers': num_layers, 'hidden_size': hidden_size
        },
    ...
    with open(os.path.join(save_dir,
            f'tcn_adv_lnn2_{appliance_name.replace(" ", "_")}_history.json'),
...
def run_all_appliances(window_size=100, num_channels=None, kernel_size=3,
                       dropout=0.2, hidden_size=64, dt=0.1, num_layers=2,
                       epochs=80, lr=0.001, patience=20):
    if num_channels is None:
        num_channels = [32, 64, 128]
    ...
    base_save_dir = f"models/tcn_adv_lnn2_redd_{timestamp}"
    ...
    for appliance_name in appliances:
        print(f"\n{'='*60}")
        print(f"TCN-AdvLNN2: {appliance_name}")
        print(f"{'='*60}\n")
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir,
                        f"tcn_adv_lnn2_{appliance_name.replace(' ', '_')}_best.pth"),
                    'final_metrics': {k: float(v) for k, v in test_metrics.items()}
                }
                print(f"Successfully tested TCN-AdvLNN2 on {appliance_name}")
    ...
    print(f"\nTCN-AdvLNN2 testing completed. Results saved to {base_save_dir}")
    return all_results


if __name__ == "__main__":
    print("Testing TCN + AdvancedLiquidNetworkModelTwo on REDD dataset...")
    ...
    print(f"\nSummary of TCN-AdvLNN2 on REDD:")
