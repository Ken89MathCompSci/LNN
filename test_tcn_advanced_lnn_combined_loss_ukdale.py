...
class CombinedLoss(nn.Module):
    """
    L_total = L_MSE + lambda * L_BCE_weighted

    L_BCE_weighted uses pos_weight to address class imbalance —
    the ON class is rare for most appliances, so we upweight it.
    ...
    def __init__(self, pos_weight=10.0, bce_lambda=0.1):
        ...
        self.mse = nn.MSELoss()
        self.pos_weight = pos_weight  # set dynamically before training

    def forward(self, outputs, targets, threshold_scaled):
        # Regression loss
        loss_mse = self.mse(outputs, targets)

        # Binary labels: 1 if above scaled threshold, 0 otherwise
        binary_targets = (targets >= threshold_scaled).float()

        # Sigmoid maps regression output to (0,1) probability — avoids treating
        # scaled power values as logits which would blow up MAE/SAE
        probs = torch.sigmoid(outputs)
        pw = torch.tensor([self.pos_weight], device=outputs.device)
        bce_fn = nn.BCELoss(weight=pw * binary_targets + (1 - binary_targets))
        loss_bce = bce_fn(probs, binary_targets)
...
def compute_pos_weight(y_train_scaled, threshold_scaled, max_pos_weight=20.0):
    """Compute pos_weight = num_off / num_on from training labels."""
    num_on  = (y_train_scaled >= threshold_scaled).sum()
    num_off = (y_train_scaled <  threshold_scaled).sum()
    if num_on == 0:
        return max_pos_weight
    return float(min(num_off / num_on, max_pos_weight))
...
def train_on_appliance(data_dict, appliance_name, window_size=100,
    ...
    # Scaled threshold for binary labels in loss
    ...
    pos_weight = compute_pos_weight(y_train, threshold_scaled)
    ...
    criterion = CombinedLoss(pos_weight=pos_weight, bce_lambda=bce_lambda)
    mse_only  = nn.MSELoss()  # for val loss tracking (consistent with other scripts)
    ...
    best_model_path = os.path.join(
        save_dir,
        f"tcn_advanced_lnn_combined_loss_ukdale_{appliance_name.replace(' ', '_')}_best.pth")

    print(f"Starting TCN-Advanced-LNN (Combined Loss) training for {appliance_name}...")
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for inputs, targets in val_loader:
                inputs, targets = inputs.to(device), targets.to(device)
                outputs = model(inputs)
                # Track MSE-only val loss for early stopping (comparable to other scripts)
    ...
    plt.savefig(os.path.join(save_dir,
        f"tcn_advanced_lnn_combined_loss_ukdale_{appliance_name.replace(' ', '_')}_metrics.png"),
    ...
    config = {
        ...
        'loss_params': {'bce_lambda': bce_lambda, 'pos_weight': pos_weight},
    ...
    with open(os.path.join(save_dir,
            f'tcn_advanced_lnn_combined_loss_ukdale_{appliance_name.replace(" ", "_")}_history.json'),
...
def test_on_all_appliances(window_size=100, hidden_size=64, num_layers=2, dt=0.1,
    ...
    base_save_dir = f"models/tcn_advanced_lnn_combined_loss_ukdale_test_{timestamp}"
    ...
    for appliance_name in appliances:
        print(f"\n{'='*60}")
        print(f"Testing TCN-Advanced-LNN (Combined Loss) on {appliance_name}")
        print(f"{'='*60}\n")
    ...
    summary = {
        ...
        'dataset': 'UKDALE',
        'loss': 'MSE + weighted BCE',
        'loss_params': {'bce_lambda': bce_lambda},
        'dataset_splits': {
    ...
    print(f"\nTCN-Advanced-LNN (Combined Loss) UKDALE testing completed. "
          f"Results saved to {base_save_dir}")
    return all_results


if __name__ == "__main__":
    print("Testing TCN-Advanced-LNN with Combined Loss on UKDALE dataset...")
    ...
    print(f"\nSummary of TCN-Advanced-LNN (Combined Loss) testing on UKDALE dataset:")
