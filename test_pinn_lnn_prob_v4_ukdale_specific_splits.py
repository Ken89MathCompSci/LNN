"""
Probabilistic PINN-LNN v4 — UKDALE specific splits
====================================================
Extends v3 with one targeted architectural fix for microwave always-OFF.

Root cause of v2/v3 MW failure
--------------------------------
With gated output mu_final = p_on × mu_power, the GaussianNLL on 95%-OFF
microwave samples pushes p_on → 0 (mu_final → 0 ≈ y_true=0 for OFF) much
more strongly than BCE with pos_weight=15 pushes it toward 1 for ON samples.
The duty-cycle ratio is ~19:1; pos_weight=15 only partially compensates.
Raising BCE_LAMBDA (v3: 0.1 → 0.4) amplified both directions proportionally —
the negative class still dominated and p_on actually fell from 0.12 to 0.08.
No hyperparameter tuning can fix a structural gradient imbalance of this kind.

Fix — un-gate microwave (index 2):
    p_on_gated = p_on.clone()
    p_on_gated[:, MW_IDX] = 1.0    # MW output = mu_power directly
    mu_final = p_on_gated * mu_power

    This degrades to pure regression for MW (same as plain PINN-LNN).
    Plain log1p PINN-LNN achieved MW F1=0.205 with this approach.
    DW, FR, WD remain gated — the sparsity prior that fixed always-ON.

    The MW state_head still trains via BCE (gives p_on monitoring signal
    and shapes the shared encoder), but BCE does NOT flow through mu_final.
    BCE_LAMBDA[MW] reverted to 0.1 (auxiliary only).

Loss (unchanged):
    Warmup:  L = GaussianNLL(mu_final, y, σ²) + Σ λ_i·BCE(logit_i, y_bin_i)
    F1 phase: +λ_phys·ProbPhysics

Early stopping (two-phase):
    Warmup → best val NLL
    F1 phase → reset, best avg val F1
"""
...
MW_IDX     = APPLIANCES.index('microwave')   # 2 — un-gated in forward()
...
# MW BCE_LAMBDA reverted to 0.1 (auxiliary: shapes encoder, no effect on mu_final)
...
class ProbabilisticPhysicsLoss(nn.Module):
    ...
    def forward(self, x_mid_scaled, mu_scaled, sigma_scaled):
        ...
        x_raw = torch.expm1(x_log.clamp(min=0))

        mu_log = mu_scaled * self.y_ranges + self.y_mins
        mu_raw = torch.expm1(mu_log.clamp(min=0))

        sigma_raw = sigma_scaled * self.y_ranges * (1.0 + mu_raw)
...
# ---------------------------------------------------------------------------
# Model: hybrid gated / ungated probabilistic PINN-LNN
# ---------------------------------------------------------------------------
...
class GatedProbPINNLiquidNetworkModel(nn.Module):
    ...
    Gating strategy (v4):
        DW, FR, WD  →  mu_final = p_on × mu_power    (sparsity prior)
        MW (idx 2)  →  mu_final = mu_power            (pure regression)

    The MW state_head still trains via BCE for monitoring and encoder shaping,
    but p_on[MW] is forced to 1.0 before computing mu_final.
    """
    ...
    def forward(self, x):
        ...
        # Un-gate MW: force p_on=1 for index MW_IDX → pure regression output
        p_on_gated            = p_on.clone()
        p_on_gated[:, MW_IDX] = 1.0
        mu_final              = p_on_gated * mu_power

        return mu_final, sigma, state_logits, p_on   # p_on still has raw MW prob
...
def train_pinn_prob_v4_model(data_dict, save_dir,
                              hidden_size=64, dt=0.1,
                              lambda_phys=LAMBDA_PHYS, epsilon_w=EPSILON_W):
    os.makedirs(save_dir, exist_ok=True)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Using device: {device}")
    print(f"Model: Gated Prob-PINN-LNN v4  λ_phys={lambda_phys}  ε={epsilon_w}W  "
          f"hidden={hidden_size}  dt={dt}")
    print(f"Scaling: log1p → MinMax")
    print(f"v4: MW (idx {MW_IDX}) un-gated (pure regression);  "
          f"DW/FR/WD gated (p_on × mu_power)")
    ...
    print("Starting Gated Prob-PINN-LNN v4 training...")
    ...
    for epoch in range(EPOCHS):
        ...
        for app in APPLIANCES:
            ...
            sig = mean_sigma[app]
            pon = mean_p_on[app]
            ungated = ' [ungated]' if app == 'microwave' else ''
            print(f"    {app:<14}  F1={m['f1']:.4f}  "
                  f"P={m['precision']:.4f}  R={m['recall']:.4f}  "
                  f"MAE={m['mae']:.2f}  SAE={m['sae']:.4f}  "
                  f"sigma={sig:.4f}  p_on={pon:.4f}{ungated}")
    ...
    config = {
        ...
        'version':  'prob_v4',
        'description': (
            'Prob-PINN-LNN v4: hybrid gating — DW/FR/WD gated (p_on × mu_power), '
            'MW un-gated (pure regression, p_on forced to 1); '
            'sigma clamped [0.01, 5.0]; log1p + MinMax scaling; '
        ...
        'gating': {app: ('ungated' if app == 'microwave' else 'gated') for app in APPLIANCES},
        'window_size': WIN,
        'model_params': {
            'input_size': 1, 'hidden_size': hidden_size,
            'n_appliances': len(APPLIANCES), 'dt': dt,
    ...
    with open(os.path.join(save_dir, 'pinn_lnn_prob_v4_ukdale_results.json'),
...
def _plot_training(history, test_metrics, save_dir):
    ...
    fig.suptitle('Gated Prob-PINN-LNN v4 — Training Losses', fontsize=12)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_prob_v4_ukdale_loss.png'),
                dpi=150, bbox_inches='tight')
    plt.close()

    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 4))
    fig.suptitle('Gated Prob-PINN-LNN v4 — Uncertainty & State Monitoring', fontsize=12)

    for i, app in enumerate(APPLIANCES):
        sig_series = [m[app] for m in history['val_sigma']]
        pon_series = [m[app] for m in history['val_p_on']]
        lbl = f'{app} [ungated]' if app == 'microwave' else app
        ax1.plot(epochs_x, sig_series, label=lbl, color=colors[i], linewidth=1.5)
        ax2.plot(epochs_x, pon_series, label=lbl, color=colors[i], linewidth=1.5)

    ax1.set_title('Val mean σ'); ax1.set_xlabel('Epoch'); ax1.set_ylabel('σ (scaled)')
    ax1.legend(fontsize=8); ax1.grid(True, alpha=0.3)

    ax2.set_title('Val mean p_on (MW is auxiliary — not used for gating)')
    ax2.set_xlabel('Epoch'); ax2.set_ylabel('mean p_on')
    ax2.legend(fontsize=8); ax2.grid(True, alpha=0.3)

    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_prob_v4_ukdale_monitoring.png'),
                dpi=150, bbox_inches='tight')
    plt.close()

    fig, axes = plt.subplots(len(APPLIANCES), 2,
                             figsize=(12, 4 * len(APPLIANCES)))
    fig.suptitle('Gated Prob-PINN-LNN v4 UKDALE — Val Metrics', fontsize=13)

    for row, app in enumerate(APPLIANCES):
        f1_series  = [m[app]['f1']  for m in history['val_metrics']]
        mae_series = [m[app]['mae'] for m in history['val_metrics']]

        title_sfx = ' [ungated]' if app == 'microwave' else ''
        ...
        axes[row][0].set_title(f'{app}{title_sfx} — F1')
        ...
        axes[row][1].set_title(f'{app}{title_sfx} — MAE (W)')
        axes[row][1].set_xlabel('Epoch'); axes[row][1].set_ylabel('MAE (W)')
        axes[row][1].legend(); axes[row][1].grid(True, alpha=0.3)

    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_prob_v4_ukdale_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
def _plot_uncertainty(y_true, y_mu, y_sigma, y_p_on, y_scalers, save_dir):
    ...
    fig.suptitle('Gated Prob-PINN-LNN v4 — Test Uncertainty Analysis', fontsize=13)
    ...
    for row, app in enumerate(APPLIANCES):
        ...
        axes[row][2].set_title(f'{app}{tag} — p_on distribution (auxiliary for MW)')
        axes[row][2].set_xlabel('p_on'); axes[row][2].set_ylabel('Count')
        axes[row][2].legend(); axes[row][2].grid(True, alpha=0.3)

    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_prob_v4_ukdale_uncertainty.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    save_dir  = f"models/pinn_lnn_prob_v4_ukdale_{timestamp}"

    data_dict = load_data()

    test_metrics, history, test_sigmas, test_p_ons = train_pinn_prob_v4_model(
        data_dict,
        save_dir    = save_dir,
        hidden_size = 64,
        dt          = 0.1,
