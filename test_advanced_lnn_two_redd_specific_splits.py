...
from models import AdvancedLiquidNetworkModelTwo
...
def train_on_appliance(data_dict, appliance_name, window_size=100,
                       ...
                       save_dir='models/advanced_lnn_two_redd_specific'):
    ...
    model = AdvancedLiquidNetworkModelTwo(
        input_size=1,
    ...
    print(f"Starting Advanced-LNN-Two (inter+intra LayerNorm) training for {appliance_name}...")
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"advanced_lnn_two_redd_{appliance_name.replace(' ', '_')}_best.pth")
            save_model(model,
    ...
    plt.savefig(os.path.join(save_dir,
        f"advanced_lnn_two_redd_{appliance_name.replace(' ', '_')}_metrics.png"),
    ...
    config = {
        ...
        'dataset': 'REDD',
        'model': 'AdvancedLiquidNetworkModelTwo (inter+intra LayerNorm)',
    ...
    with open(os.path.join(save_dir,
            f'advanced_lnn_two_redd_{appliance_name.replace(" ", "_")}_history.json'),
...
def test_on_all_appliances(window_size=100, hidden_size=64, num_layers=2, dt=0.1,
    ...
    base_save_dir = f"models/advanced_lnn_two_redd_specific_test_{timestamp}"
    ...
    for appliance_name in appliances:
        print(f"\n{'='*60}")
        print(f"Testing Advanced-LNN-Two on {appliance_name}")
        print(f"{'='*60}\n")
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir,
                        f"advanced_lnn_two_redd_{appliance_name.replace(' ', '_')}_best.pth"),
    ...
    summary = {
        ...
        'model': 'AdvancedLiquidNetworkModelTwo (inter+intra LayerNorm)',
        'dataset_splits': {
            'training':   {'house': 3, 'period': '1 day'},
            'validation': {'house': 3, 'period': '1 day'},
            'testing':    {'house': 1, 'period': '~33 hours'}
        },
        'window_size': window_size,
    ...
    print(f"\nAdvanced-LNN-Two REDD testing completed. Results saved to {base_save_dir}")
    return all_results


if __name__ == "__main__":
    print("Testing Advanced-LNN-Two (inter+intra LayerNorm) on REDD dataset with specific splits...")
    ...
    print(f"\nSummary of Advanced-LNN-Two testing on REDD dataset:")
