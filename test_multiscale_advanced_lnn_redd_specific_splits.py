...
def train_on_appliance(data_dict, appliance_name, save_dir,
    ...
    plt.savefig(os.path.join(save_dir,
        f"multiscale_advanced_lnn_redd_{appliance_name.replace(' ', '_')}_metrics.png"),
    ...
    with open(os.path.join(save_dir,
            f'multiscale_advanced_lnn_redd_{appliance_name.replace(" ", "_")}_history.json'),
...
def run_all(hidden_size=64, dt_fast=DT_FAST, dt_med=DT_MED, dt_slow=DT_SLOW):
    ...
    base_save_dir = f"models/multiscale_advanced_lnn_redd_test_{timestamp}"

    all_results = {}

    for appliance_name in APPLIANCES:
        print(f"\n{'='*60}")
        print(f"Testing Multi-Scale-Advanced-LNN on {appliance_name} (REDD)")
        print(f"  dt_fast={dt_fast}  dt_med={dt_med}  dt_slow={dt_slow}")
        print(f"{'='*60}\n")
    ...
    print(f"\nMulti-Scale-Advanced-LNN REDD testing completed.")
