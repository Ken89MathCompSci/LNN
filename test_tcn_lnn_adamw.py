"""
CNN + TCN + LNN Hybrid — 80 Epochs (AdamW)
============================================
Trains and evaluates TCNLNNModel on all REDD appliances.
Identical to test_tcn_lnn.py but uses AdamW (weight_decay=1e-4) instead of Adam.
...
Usage:
    !python test_tcn_lnn_adamw.py
    !python test_tcn_lnn_adamw.py --epochs 80 --hidden 256
"""
...
SAVE_DIR = os.path.join('results', 'tcn_lnn_adamw')
COLOR    = '#2CA02C'
...
def plot_training_curves(results, save_dir):
    ...
    fig.suptitle('Training Curves — CNN + TCN + LNN AdamW (80 epochs)', fontsize=12)
    plt.tight_layout()
...
def plot_epoch_metrics(results, save_dir):
    ...
    for row, app in enumerate(APPLIANCES):
        ...
        for col, (hist_key, mk_label) in enumerate(metric_info):
            ...
            ax.grid(True, alpha=0.3)
    fig.suptitle('Val Metrics per Epoch — CNN + TCN + LNN AdamW (80 epochs)', fontsize=12)
    plt.tight_layout()
...
def plot_bar_chart(results, save_dir):
    ...
    fig.suptitle('Final Metrics — CNN + TCN + LNN AdamW (80 epochs)', fontsize=12)
    plt.tight_layout()
...
def print_table(results):
    ...
    for metric, label in [('f1', 'F1'), ('mae', 'MAE'), ('sae', 'SAE')]:
        print(f"\n{'='*75}")
        print(f"  {label} — CNN + TCN + LNN (AdamW)")
        print(f"{'='*75}")
...
def main():
    parser = argparse.ArgumentParser(description='CNN + TCN + LNN (AdamW) — 80 Epochs')
    ...
    print('\n' + '=' * 70)
    print('  CNN + TCN + LNN (AdamW) — Training')
    print('=' * 70)
