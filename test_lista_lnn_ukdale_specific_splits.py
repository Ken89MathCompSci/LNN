"""
LISTA-LNN (Learned ISTA / Sparse Coding) for NILM — UKDALE specific splits.
...
Loss (three terms):
    L = MSE(p̂, p*)                     — power regression
      ...
      + λ_bce   · BCE(σ(p̂ / thr), s*) — on/off supervision
...
Trained per-appliance (same structure as test_lnn_ukdale_specific_splits.py).
"""
...
WIN         = 100
STRIDE      = 5
N_ATOMS     = 64     # sparse code dimension
K_LAYERS    = 8      # number of unrolled ISTA iterations
...
class LISTALayer(nn.Module):
    ...
    We  : (signal_len → n_atoms)  encodes the input signal y
    Wr  : (n_atoms    → n_atoms)  recurrent update from previous x
    threshold: learnable per-atom soft-threshold (initialised small)
    """
    ...
    def forward(self, y: torch.Tensor, x_prev: torch.Tensor) -> torch.Tensor:
        """
        Args:
            y      : (batch, signal_len) — input signal (fixed across layers)
            x_prev : (batch, n_atoms)    — sparse code from previous layer
        Returns:
            x_new  : (batch, n_atoms)
        """
...
class LISTANILMModel(nn.Module):
    """
    K unrolled LISTA layers + per-appliance linear decoder.

    Dictionary D (n_atoms × signal_len) is learned jointly for the
    reconstruction loss  ||y - x @ D||².
    """
    ...
    def __init__(self, signal_len: int = WIN, n_atoms: int = N_ATOMS,
        ...
        # Per-appliance power head
        self.head = nn.Linear(n_atoms, 1)

    def forward(self, y: torch.Tensor):
        """
        Args:
            y : (batch, WIN, 1) or (batch, WIN) — mains window
        Returns:
            power : (batch, 1)  — appliance power estimate (scaled)
            x     : (batch, n_atoms) — final sparse codes
        """
        if y.dim() == 3:
            y = y.squeeze(-1)                   # (batch, WIN)

        x = torch.zeros(y.size(0), self.n_atoms, device=y.device)
        for layer in self.layers:
            x = layer(y, x)

        power = self.head(x)                    # (batch, 1)
        return power, x

    def reconstruct(self, x: torch.Tensor) -> torch.Tensor:
        """Reconstruct signal from sparse codes: y_hat = x @ D."""
        return x @ self.D                       # (batch, WIN)
...
def create_sequences(data, appliance_name, window_size=WIN):
    """Midpoint targeting — label is appliance power at window centre."""
...
def train_on_appliance(data_dict, appliance_name,
    ...
    tr_loader = torch.utils.data.DataLoader(
        UKDALEDataset(X_tr, y_tr), batch_size=BATCH, shuffle=True,  drop_last=False)
    va_loader = torch.utils.data.DataLoader(
        UKDALEDataset(X_va, y_va), batch_size=BATCH, shuffle=False, drop_last=False)
    te_loader = torch.utils.data.DataLoader(
        UKDALEDataset(X_te, y_te), batch_size=BATCH, shuffle=False, drop_last=False)
    ...
    for epoch in range(epochs):
        ...
        for xb, yb in bar:
            ...
            recon_loss  = F.mse_loss(y_hat, y_flat)
        ...
        if avg_va_loss < best_val_loss:
            ...
            save_model(
                ...
                os.path.join(save_dir,
                    f"lista_lnn_ukdale_{appliance_name.replace(' ', '_')}_best.pth")
            )
        else:
            counter += 1
            if counter >= patience:
    ...
    with open(os.path.join(save_dir,
            f'lista_lnn_ukdale_{appliance_name.replace(" ", "_")}_history.json'),
...
def _plot_results(history, test_metrics, appliance_name, save_dir):
    ...
    plt.savefig(os.path.join(save_dir,
        f"lista_lnn_ukdale_{appliance_name.replace(' ', '_')}_metrics.png"),
...
def test_on_all_appliances(n_atoms=N_ATOMS, k_layers=K_LAYERS,
    ...
    print(f"\nLISTA-LNN UKDALE testing complete. Results saved to {base_save_dir}\n")
