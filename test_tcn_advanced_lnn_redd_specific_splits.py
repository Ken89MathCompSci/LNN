...
def train_on_appliance(splits, appliance_name, save_dir,
    ...
    print(f"Starting TCN-Advanced-LNN (MSE Loss) training for {appliance_name}...")
    ...
    plt.savefig(os.path.join(save_dir,
        f"tcn_advanced_lnn_redd_{appliance_name.replace(' ', '_')}_metrics.png"),
    ...
    with open(os.path.join(save_dir,
            f'tcn_advanced_lnn_redd_{appliance_name.replace(" ", "_")}_history.json'),
...
def run_all(hidden_size=64, num_layers=2, dt=0.1,
    ...
    base_save_dir = f"models/tcn_advanced_lnn_redd_test_{timestamp}"

    all_results = {}

    for appliance_name in APPLIANCES:
        print(f"\n{'='*60}")
        print(f"Testing TCN-Advanced-LNN (MSE Loss) on {appliance_name}")
        print(f"{'='*60}\n")
    ...
    print(f"\nTCN-Advanced-LNN (MSE Loss) REDD testing completed.")
