"""
Physics-Informed LNN (PINN-LNN) + Temporal Smoothness — UKDALE specific splits.

Extends test_pinn_lnn_ukdale_specific_splits.py with a temporal smoothness
regularisation term:

    L_total = L_MSE + λ_phys * L_phys + λ_smooth * L_smooth + L_BCE

    L_smooth = mean( (P̂(t) - P̂(t-1))² )   over consecutive window predictions
             = mean of squared first differences across consecutive time windows

Why smoothness helps in NILM:
    - Appliances like fridge and washer dryer have slow, sustained power profiles.
      Without smoothness regularisation the model can produce jittery, noisy
      predictions that flip ON/OFF every few timesteps.
    - The L2 smoothness penalty penalises large prediction jumps, encouraging
      stable trajectories that better match real appliance behaviour.
    - Particularly beneficial for F1, SAE, and visual quality on slow appliances.

Implementation note — sequential training loader:
    Smoothness is computed across consecutive items in the training batch.
    To ensure consecutive batch items correspond to consecutive windows in time
    the training DataLoader uses shuffle=False (sequential loading).
    Windows overlap heavily (WIN=100, STRIDE=5) so sequential loading still
    provides sufficient sample diversity.

Other improvements over base:
    Two-phase early stopping — warmup tracks best val MSE; after WARMUP_EPOCHS
    the counter resets and tracks best avg val F1 (so BCE training is retained).
"""
...
def train_pinn_model(data_dict, save_dir,
    ...
    print(f"Scaling: standard MinMax  [smooth: λ_smooth={lambda_smooth}, λ_phys={lambda_phys}]")
    ...
    print("Starting PINN-LNN (smooth) training...")
    ...
    for epoch in range(EPOCHS):
        ...
        for xb, yb in progress_bar:
            ...
            if epoch < WARMUP_EPOCHS:
                # Warmup: MSE + smoothness (no physics / BCE yet)
    ...
    config = {
        ...
        'model': 'PhysicsInformedLiquidNetworkModel',
        'description': (
            'PINN-LNN + temporal smoothness regularisation; '
            'sequential training loader; two-phase early stopping'
        ),
        'loss': (
            f'MSE + {lambda_phys} * PhysicsConsistency(ε={epsilon_w}W) '
    ...
    with open(os.path.join(save_dir, 'pinn_lnn_smooth_ukdale_results.json'),
...
def _plot_training(history, test_metrics, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_smooth_ukdale_loss.png'),
    ...
    fig.suptitle('PINN-LNN (smooth) UKDALE — Per-Appliance Val Metrics', fontsize=13)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_smooth_ukdale_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    save_dir  = f"models/pinn_lnn_smooth_ukdale_{timestamp}"
