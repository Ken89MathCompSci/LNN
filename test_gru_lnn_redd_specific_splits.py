...
from models import GRULiquidNetworkModel
...
def train_gru_lnn_on_appliance(data_dict, appliance_name, window_size=100,
                                gru_hidden=64, num_gru_layers=2,
                                hidden_size=64, dropout=0.2, dt=0.1,
                                epochs=80, lr=0.001, patience=20,
                                save_dir='models/gru_lnn_redd_specific'):
    ...
    train_loader = torch.utils.data.DataLoader(
        REDDSpecificDataset(X_train, y_train), batch_size=32, shuffle=True)
    val_loader = torch.utils.data.DataLoader(
        REDDSpecificDataset(X_val, y_val), batch_size=32, shuffle=False)
    test_loader = torch.utils.data.DataLoader(
        REDDSpecificDataset(X_test, y_test), batch_size=32, shuffle=False)

    input_size  = 1
    output_size = 1

    model = GRULiquidNetworkModel(
    ...
    print(f"Starting GRU-LNN training for {appliance_name}...")
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"gru_lnn_redd_{appliance_name.replace(' ', '_')}_best.pth")
            save_model(model,
    ...
    plt.savefig(os.path.join(save_dir,
        f"gru_lnn_redd_{appliance_name.replace(' ', '_')}_metrics.png"),
    ...
    with open(os.path.join(save_dir,
            f'gru_lnn_redd_{appliance_name.replace(" ", "_")}_history.json'),
...
def test_gru_lnn_on_all_redd_appliances(window_size=100, gru_hidden=64,
    ...
    base_save_dir = f"models/gru_lnn_redd_specific_test_{timestamp}"
    ...
    for appliance_name in appliances:
        print(f"\n{'='*60}")
        print(f"Testing GRU-LNN on {appliance_name}")
        print(f"{'='*60}\n")
        ...
        try:
            model, history, test_metrics = train_gru_lnn_on_appliance(
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir,
                        f"gru_lnn_redd_{appliance_name.replace(' ', '_')}_best.pth"),
                    'final_metrics': {k: float(v) for k, v in test_metrics.items()}
                }
                print(f"Successfully tested GRU-LNN on {appliance_name}")
    ...
    print(f"\nGRU-LNN REDD testing completed. Results saved to {base_save_dir}")
    return all_results


if __name__ == "__main__":
    print("Testing GRU-LNN on REDD dataset with specific splits...")
    ...
    results = test_gru_lnn_on_all_redd_appliances(
        window_size=100,
        gru_hidden=64,
        num_gru_layers=2,
        hidden_size=64,
    ...
    print(f"\nSummary of GRU-LNN testing on REDD dataset:")
