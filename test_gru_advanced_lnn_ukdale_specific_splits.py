...
from models import GRUAdvancedLiquidNetworkModel
...
def train_gru_advanced_lnn_on_appliance(data_dict, appliance_name, window_size=100,
                                         gru_hidden=64, num_gru_layers=2,
                                         hidden_size=64, dropout=0.2, dt=0.1,
                                         num_layers=2,
                                         epochs=80, lr=0.001, patience=20,
                                         save_dir='models/gru_advanced_lnn_ukdale_specific'):
    ...
    model = GRUAdvancedLiquidNetworkModel(
    ...
    best_model_path = os.path.join(
        save_dir,
        f"gru_advanced_lnn_ukdale_{appliance_name.replace(' ', '_')}_best.pth")

    print(f"Starting GRU-Advanced-LNN training for {appliance_name}...")
    ...
    plt.savefig(os.path.join(save_dir,
        f"gru_advanced_lnn_ukdale_{appliance_name.replace(' ', '_')}_metrics.png"),
    ...
    with open(os.path.join(save_dir,
            f'gru_advanced_lnn_ukdale_{appliance_name.replace(" ", "_")}_history.json'),
...
def test_gru_advanced_lnn_on_all_ukdale_appliances(window_size=100, gru_hidden=64,
                                                    num_gru_layers=2, hidden_size=64,
                                                    dropout=0.2, dt=0.1, num_layers=2,
                                                    epochs=80, lr=0.001, patience=20):

    data_dict = load_ukdale_specific_splits()
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    base_save_dir = f"models/gru_advanced_lnn_ukdale_specific_test_{timestamp}"
    ...
    for appliance_name in appliances:
        print(f"\n{'='*60}")
        print(f"Testing GRU-Advanced-LNN on {appliance_name}")
        print(f"{'='*60}\n")
        ...
        try:
            model, history, test_metrics = train_gru_advanced_lnn_on_appliance(
                data_dict, appliance_name=appliance_name,
                window_size=window_size, gru_hidden=gru_hidden,
                num_gru_layers=num_gru_layers, hidden_size=hidden_size,
                dropout=dropout, dt=dt, num_layers=num_layers,
            ...
            if model is not None:
                ...
                print(f"Successfully tested GRU-Advanced-LNN on {appliance_name}")
    ...
    print(f"\nGRU-Advanced-LNN UKDALE testing completed. Results saved to {base_save_dir}")
    return all_results


if __name__ == "__main__":
    print("Testing GRU-Advanced-LNN on UKDALE dataset with specific splits...")
    ...
    results = test_gru_advanced_lnn_on_all_ukdale_appliances(
        window_size=100,
        gru_hidden=64,
        num_gru_layers=2,
        hidden_size=64,
    ...
    print(f"\nSummary of GRU-Advanced-LNN testing on UKDALE dataset:")
