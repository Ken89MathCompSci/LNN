"""
Physics-Informed LNN (PINN-LNN) + log1p scaling v2 — UKDALE specific splits
=============================================================================
Extends test_pinn_lnn_log_ukdale_specific_splits.py with two fixes:

    Fix 1 — Enable BCE for microwave
        BCE_LAMBDA['microwave'] = 0.1  (was 0.0)
        BCE_ALPHA ['microwave'] = 3.0  (was 1.0)
        Microwave is ON only ~1% of the time; without a class-imbalance
        signal the model never learns to predict it.

    Fix 2 — Two-phase early stopping (warmup → F1)
        Warmup phase  (epoch < WARMUP_EPOCHS) : track best val_mse
        BCE    phase  (epoch ≥ WARMUP_EPOCHS) : reset counter, track best avg val F1
        Without this the saved model is always from the warmup phase (epoch 18
        in v1), meaning BCE training has zero effect on the saved weights.

Everything else is identical to test_pinn_lnn_log_ukdale_specific_splits.py.

Original description:
Identical to test_pinn_lnn_ukdale_specific_splits.py except the MinMaxScaler
is replaced with a  log1p → MinMax  two-step pipeline.

Why log1p for NILM
    Power distributions are heavily right-skewed:
        • Fridge  : mostly ~100 W, rare compressor spikes
        • Microwave: 0 W most of the time, then 800–1000 W briefly
        • Mains   : long tail up to several kW

    MinMax on raw Watts compresses small loads and rare spikes into a
    narrow band near zero.  This makes:
        - the MSE gradient dominated by large loads (washer dryer)
        - microwave and brief events nearly invisible in loss space
        - the physics constraint numerically sensitive to outliers

    log1p ( x' = log(1+x) ) compresses the tail and expands the low-
    power region:
        x = 0 W    →  x' = 0.000   (zero stays zero)
        x = 10 W   →  x' = 2.398
        x = 100 W  →  x' = 4.615
        x = 1000 W →  x' = 6.909
        x = 5000 W →  x' = 8.517

    After log1p the distribution is closer to uniform, so MinMaxScaler
    actually spreads values across [0, 1] without compressing small loads.

Scaling pipeline  (applied to both mains X and appliance Y)
    raw                     →  log1p  →  MinMaxScaler  →  model input / target
    model output (scaled)   →  MinMaxScaler⁻¹  →  expm1  →  raw Watts

Five places where scaling is touched vs the base script:
    1. X / Y array preparation — np.log1p before MinMaxScaler.fit_transform
    2. thresholds_scaled      — np.log1p(threshold_W) before linear scaling
    3. PhysicsConsistencyLoss.forward — torch.expm1 after MinMax⁻¹ inverse
    4. compute_per_appliance_metrics  — np.expm1  after MinMax⁻¹ inverse
    5. JSON / print labels    — updated to note log1p scaling

Everything else (architecture, loss schedule, BCE, early stopping, plots)
is unchanged from the base script.

Architecture:
    Input (batch, WIN, 1)  — log1p-scaled mains window
         ↓
    ...
    └────┴────┴────┴────┘
    output: (batch, 4)  →  MinMaxScaler⁻¹  →  expm1  →  raw Watts
"""
...
LAMBDA_PHYS   = 0.01
EPSILON_W     = 50.0   # tolerance in raw Watts (physics constraint)
WARMUP_EPOCHS = 20
...
BCE_LAMBDA = {'dish washer': 0.5, 'fridge': 0.3, 'microwave': 0.1, 'washer dryer': 0.0}
BCE_ALPHA  = {'dish washer': 2.0, 'fridge': 2.0, 'microwave': 3.0, 'washer dryer': 1.0}


# ---------------------------------------------------------------------------
# Physics Consistency Loss  (log1p-aware inverse scaling)
# ---------------------------------------------------------------------------
...
class PhysicsConsistencyLoss(nn.Module):
    ...
    The model outputs are in log1p-MinMax space.  Inverse pipeline:
        scaled → MinMax⁻¹ (linear) → log-space → expm1 → raw Watts

    All ops are differentiable so gradients flow back through the inverse.
    """
    ...
    def __init__(self, x_scaler, y_scalers, appliances, epsilon_w=EPSILON_W):
        ...
        # Mains: MinMax params fitted on log1p(X)
        ...
        # Per-appliance: MinMax params fitted on log1p(Y_i)
    ...
    def forward(self, x_mid_scaled, pred_scaled):
        ...
        pred_scaled  : (batch, n_apps) — log1p-MinMax scaled appliance preds
        Returns: scalar physics violation loss
        """
        # Step 1: MinMax⁻¹  →  log space
        x_log = x_mid_scaled * self.x_range + self.x_min          # (batch,)
        p_log = pred_scaled  * self.y_ranges + self.y_mins         # (batch, n_apps)

        # Step 2: expm1  →  raw Watts  (clamp prevents log-domain negatives)
        x_raw = torch.expm1(x_log.clamp(min=0.0))                 # (batch,)
        p_raw = torch.expm1(p_log.clamp(min=0.0))                 # (batch, n_apps)
...
# ---------------------------------------------------------------------------
# Physics-Informed LNN Model  (unchanged from base)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Per-appliance metrics  (expm1 inverse of log1p scaling)
# ---------------------------------------------------------------------------

def compute_per_appliance_metrics(y_true, y_pred, y_scalers):
    """
    y_true, y_pred : (N, n_appliances) — log1p-MinMax scaled
    Inverse pipeline: MinMax⁻¹ → expm1 → raw Watts → metrics
    """
    metrics = {}
    for i, app in enumerate(APPLIANCES):
        log_true = y_scalers[i].inverse_transform(y_true[:, i:i+1])
        log_pred = y_scalers[i].inverse_transform(y_pred[:, i:i+1])
...
def train_pinn_model(data_dict, save_dir,
    ...
    print(f"Scaling: log1p → MinMax  [v2: MW BCE enabled, two-phase early stopping]")
    ...
    # ── log1p transform BEFORE MinMaxScaler ──────────────────────────────────
    # Compresses skewed power distribution so MinMax spans the full value range
    # rather than compressing everything below the rare maximum.
    X_tr = np.log1p(X_tr);  X_va = np.log1p(X_va);  X_te = np.log1p(X_te)
    Y_tr = np.log1p(Y_tr);  Y_va = np.log1p(Y_va);  Y_te = np.log1p(Y_te)

    # ── MinMaxScaler on log-transformed values ────────────────────────────────
    ...
    # ── Thresholds in scaled space ────────────────────────────────────────────
    # Apply log1p to the raw-Watt threshold before the linear MinMax inversion
    # so the boundary sits at the correct point in log-MinMax space.
    thresholds_scaled = [
    ...
    best_val_loss = float('inf')   # warmup phase: tracks val_mse
    best_val_f1   = -1.0           # BCE phase: tracks avg val F1
    best_state    = None
    counter       = 0
    bce_phase     = False          # flips to True at WARMUP_EPOCHS

    n_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"Model parameters: {n_params:,}")
    print("Starting PINN-LNN (log1p v2) training...")
    ...
    for epoch in range(EPOCHS):
        ...
        # Warmup: save on best val_mse so the encoder learns good representations.
        # BCE phase: reset counter and save on best avg val F1 so BCE-trained
        # weights are actually captured (the saved model in v1 was always epoch 18).
        if epoch + 1 == WARMUP_EPOCHS:
            bce_phase     = True
            counter       = 0
            best_val_f1   = -1.0
            print("  [switching to BCE phase — early stopping now tracks val F1]")
    ...
    config = {
        ...
        'description': (
            'shared AdvancedLiquidTimeLayer encoder + per-appliance heads + L_phys; '
            'log1p applied to X and Y before MinMaxScaler; '
            'v2: MW BCE enabled (λ=0.1 α=3.0), two-phase early stopping (warmup→F1)'
        ),
    ...
    with open(os.path.join(save_dir, 'pinn_lnn_log_v2_ukdale_results.json'),
...
def _plot_training(history, test_metrics, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_log_v2_ukdale_loss.png'),
                dpi=150, bbox_inches='tight')
    plt.close()

    fig, axes = plt.subplots(len(APPLIANCES), 2,
                             figsize=(12, 4 * len(APPLIANCES)))
    fig.suptitle('PINN-LNN (log1p v2) UKDALE — Per-Appliance Val Metrics', fontsize=13)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_log_v2_ukdale_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    save_dir  = f"models/pinn_lnn_log_v2_ukdale_{timestamp}"
