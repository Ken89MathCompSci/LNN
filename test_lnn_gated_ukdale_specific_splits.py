...
# ── Input-gated LTC ────────────────────────────────────────────────────────────
#
#   g    = σ(W_g·x + U_g·h)           GRU-style input gate ∈ (0,1)
#   dh/dt = -h/τ + g ⊙ tanh(W·x + U·h)
#   h    ← clamp(h + dt·dh/dt, -10, 10)
#
# Improvement over basic LTC:
#   • gate g suppresses irrelevant timesteps (closes on flat baseline,
#     opens on appliance spikes) — directly fixes microwave always-OFF/ON
#   • only ~+1 linear layer vs basic LTC

class GatedLTCLayer(nn.Module):
    def __init__(self, input_size, hidden_size, dt=0.1):
        super().__init__()
        self.hidden_size = hidden_size
        self.dt          = dt

        # Input gate: g = σ(W_g·x + U_g·h)
        self.W_g = nn.Linear(input_size,  hidden_size)
        self.U_g = nn.Linear(hidden_size, hidden_size, bias=False)

        # ODE body: tanh(W·x + U·h)
        self.W   = nn.Linear(input_size,  hidden_size)
        self.U   = nn.Linear(hidden_size, hidden_size, bias=False)

        # Learnable time constant (one per neuron, always positive via softplus)
        self.tau = nn.Parameter(torch.ones(hidden_size))

    def forward(self, x, h):
        g    = torch.sigmoid(self.W_g(x) + self.U_g(h))
        f    = torch.tanh(self.W(x) + self.U(h))
        tau  = F.softplus(self.tau)
        dhdt = -h / tau + g * f
        return (h + self.dt * dhdt).clamp(-10.0, 10.0)


class GatedLTCModel(nn.Module):
    def __init__(self, input_size=1, hidden_size=64, output_size=1, dt=0.1):
        super().__init__()
        self.hidden_size = hidden_size
        self.cell        = GatedLTCLayer(input_size, hidden_size, dt)
...
def train_on_appliance(data_dict, appliance_name, window_size=100,
                       ...
                       save_dir='models/gated_lnn_ukdale_specific'):
    ...
    print(f"Starting GatedLTCModel training for {appliance_name}...")
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"gated_lnn_ukdale_{appliance_name.replace(' ', '_')}_best.pth")
            save_model(model,
    ...
    plt.savefig(os.path.join(save_dir,
        f"gated_lnn_ukdale_{appliance_name.replace(' ', '_')}_metrics.png"),
    ...
    config = {
        ...
        'dataset': 'UKDALE',
        'model': 'GatedLTCModel',
        'ode': 'g=sigma(W_g*x+U_g*h); dh/dt = -h/tau + g*tanh(W*x+U*h)',
        'norm': 'none',
        'loss': 'MSE',
    ...
    with open(os.path.join(save_dir,
            f'gated_lnn_ukdale_{appliance_name.replace(" ", "_")}_history.json'),
...
def test_on_all_appliances(window_size=100, hidden_size=64, dt=0.1,
    ...
    base_save_dir = f"models/gated_lnn_ukdale_specific_test_{timestamp}"
    ...
    for appliance_name in appliances:
        print(f"\n{'='*60}")
        print(f"Testing GatedLTCModel on {appliance_name}")
        print(f"{'='*60}\n")
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir,
                        f"gated_lnn_ukdale_{appliance_name.replace(' ', '_')}_best.pth"),
    ...
    print(f"\nGatedLTCModel UKDALE testing completed. Results saved to {base_save_dir}\n")
...
if __name__ == "__main__":
    print("Testing GatedLTCModel on UKDALE dataset with specific splits...")
    ...
    print(f"\nSummary of GatedLTCModel testing on UKDALE dataset:")
