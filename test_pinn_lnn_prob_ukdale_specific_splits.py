"""
Probabilistic Physics-Informed LNN (Prob-PINN-LNN) — UKDALE specific splits
=============================================================================
Extends PINN-LNN with heteroscedastic Gaussian uncertainty estimation.

Standard model:  y_hat = f(x)              (single deterministic prediction)
This model:      y ~ N(mu, sigma^2)         (full probability distribution)
                 mu, sigma = f(x)

Why uncertainty matters for NILM
---------------------------------
The same aggregate mains reading can be explained by many appliance combinations:
    +1200 W  →  microwave (1200W)  OR  kettle (1200W)  OR  washer heater (1200W)
A deterministic model is forced to commit; a probabilistic model says:
    mu = 1200W,  sigma = 400W   ("probably microwave, but not certain")

Key changes from test_pinn_lnn_ukdale_specific_splits.py
---------------------------------------------------------
1. Each appliance head has two outputs: mu (mean) and log_sigma (log scale)
       sigma = softplus(log_sigma) + 1e-6     (positive-definite)
   Implemented as SEPARATE mu_heads and sigma_heads so sigma bias can be
   initialised to -2.0, giving initial sigma ~ 0.13 (small, not collapsed).

2. Gaussian NLL replaces MSE as the regression loss:
       L_NLL = 0.5 * log(sigma^2) + 0.5 * (y - mu)^2 / sigma^2
   Self-calibrating: confident predictions are punished hard, uncertain ones
   are given slack — but log(sigma) prevents sigma from growing without limit.

3. Probabilistic physics loss normalises by the joint uncertainty:
       L_phys = mean( ReLU(sum_i mu_i_raw - P_agg_raw - eps)
                      / sqrt(sum_i sigma_i_raw^2) )
   Interpretation: z-score of the physics violation relative to the model's
   combined uncertainty.  Large sigma -> same excess is less penalised.
   (Under independent Gaussian appliances, var(sum) = sum(var_i).)

4. BCE applied to mu (same pseudo-probability trick as base script).

5. Two-phase early stopping:
       Warmup  (epoch < WARMUP_EPOCHS): track best val NLL
       BCE phase (epoch >= WARMUP_EPOCHS): reset counter, track best avg F1

6. Uncertainty tracked per appliance every epoch; uncertainty plots saved at
   test time (sigma histogram + |error| vs sigma scatter per appliance).

Architecture
------------
    Input (batch, WIN, 1)
         |
    Shared AdvancedLiquidTimeLayer encoder (adaptive tau, input-gated)
         |
    LayerNorm(hidden)
         |
    mu_heads (n_apps x Linear(hidden, 1))   sigma_heads (n_apps x Linear(hidden, 1))
         |                                          |
    (batch, n_apps) mu              (batch, n_apps) sigma = softplus(...)+1e-6
"""
...
# ---------------------------------------------------------------------------
# Probabilistic Physics Consistency Loss
# ---------------------------------------------------------------------------

class ProbabilisticPhysicsLoss(nn.Module):
    """
    Uncertainty-normalised one-sided physics penalty.

    Standard:     ReLU(sum_i mu_i_raw - P_agg_raw - eps)
    Probabilistic: above / sqrt(sum_i sigma_i_raw^2)

    When the model is uncertain (large sigma_i), the same physical violation
    is divided by a larger number and therefore penalised less.
    This is the z-score of the violation under the joint Gaussian of the sum.
    """
    ...
    def forward(self, x_mid_scaled, mu_scaled, sigma_scaled):
        ...
        sigma_raw = sigma_scaled * self.y_ranges                 # (batch, n_apps) — scale only

        mu_sum    = mu_raw.sum(dim=1)                            # (batch,)
        violation = F.relu(mu_sum - x_raw - self.epsilon)        # (batch,)

        sigma_sum = torch.sqrt((sigma_raw ** 2).sum(dim=1) + 1e-8)  # (batch,)
...
class ProbPINNLiquidNetworkModel(nn.Module):
    ...
    Returns:
        mu    : (batch, n_appliances)  predicted mean in scaled space
        sigma : (batch, n_appliances)  predicted std  (always > 0)
    """
    ...
    def __init__(self, input_size, hidden_size, n_appliances, dt=0.1):
        ...
        # Shared encoder — identical to base PINN-LNN
        ...
        # Per-appliance mu heads (default Xavier init)
        self.mu_heads = nn.ModuleList([
            nn.Linear(hidden_size, 1) for _ in range(n_appliances)
        ])

        # Per-appliance sigma heads — bias = -2 so initial sigma ~ 0.13
        # (slightly overconfident at start; learns uncertainty from data)
    ...
    def forward(self, x):
        ...
        mu    = torch.cat([head(h) for head in self.mu_heads],    dim=1)  # (B, n_apps)
        log_s = torch.cat([head(h) for head in self.sigma_heads], dim=1)  # (B, n_apps)
        sigma = F.softplus(log_s) + 1e-6                                   # (B, n_apps)
...
def compute_per_appliance_metrics(y_true, y_mu, y_scalers):
    """y_true, y_mu: (N, n_appliances) scaled. Metrics computed on mu only."""
    metrics = {}
...
def train_pinn_prob_model(data_dict, save_dir,
    ...
    print(f"Model: Prob-PINN-LNN  λ_phys={lambda_phys}  ε={epsilon_w}W  "
    ...
    print("Starting Prob-PINN-LNN training...")
    ...
    for epoch in range(EPOCHS):
        ...
        for xb, yb in progress_bar:
            ...
            mu, sigma = model(xb)                            # (B, n_apps) each

            nll_loss  = nll_criterion(mu, yb, sigma ** 2)
            mse_mon   = F.mse_loss(mu.detach(), yb).item()  # monitor only, not in loss
        ...
        with torch.no_grad():
            for xb, yb in va_loader:
                ...
                x_mid     = xb[:, WIN // 2, 0]
                phys_loss = phys_criterion(x_mid, mu, sigma)
                loss      = nll_loss + lambda_phys * phys_loss
        ...
        print(
            ...
            f"train={avg_tr_total:.5f} (nll={avg_tr_nll:.5f} mse={avg_tr_mse:.5f} phys={avg_tr_phys:.5f})  "
    ...
    config = {
        ...
        'model':    'ProbPINNLiquidNetworkModel',
        'description': (
            'heteroscedastic Gaussian PINN-LNN; '
            'GaussianNLL + probabilistic physics loss + BCE; '
            'two-phase early stopping (warmup→F1)'
        ),
        'loss': f'GaussianNLL + {lambda_phys} * ProbPhysics(eps={epsilon_w}W) + BCE',
        'window_size': WIN,
        'model_params': {
            'input_size': 1, 'hidden_size': hidden_size,
            'n_appliances': len(APPLIANCES), 'dt': dt,
    ...
    with open(os.path.join(save_dir, 'pinn_lnn_prob_ukdale_results.json'),
...
def _plot_training(history, test_metrics, save_dir):
    ...
    plt.subplot(1, 4, 3)
    plt.plot(epochs_x, history['train_mse'], label='Train MSE(mu)', color='blue')
    plt.plot(epochs_x, history['val_mse'],   label='Val MSE(mu)',   color='red')
    plt.title('MSE of mu (monitoring only)')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_prob_ukdale_loss.png'),
                dpi=150, bbox_inches='tight')
    plt.close()

    fig, axes = plt.subplots(len(APPLIANCES), 2,
                             figsize=(12, 4 * len(APPLIANCES)))
    fig.suptitle('Prob-PINN-LNN UKDALE — Per-Appliance Val Metrics', fontsize=13)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_prob_ukdale_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
def _plot_uncertainty(y_true, y_mu, y_sigma, y_scalers, save_dir):
    ...
    fig, axes = plt.subplots(len(APPLIANCES), 2,
                             figsize=(12, 4 * len(APPLIANCES)))
    fig.suptitle('Prob-PINN-LNN UKDALE — Test Uncertainty Analysis', fontsize=13)
    ...
    for row, app in enumerate(APPLIANCES):
        ...
        sigma_raw = y_sigma[:, i] * float(y_scalers[i].data_range_[0])  # to Watts
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_prob_ukdale_uncertainty.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    save_dir  = f"models/pinn_lnn_prob_ukdale_{timestamp}"
