"""
Conductance-based LTC — UKDALE specific splits
===============================================
...
Usage:
    python test_lnn_conductance_ukdale_specific_splits.py --appliance microwave
    python test_lnn_conductance_ukdale_specific_splits.py --plot
"""
...
SAVE_DIR = os.path.join('results', 'lnn_conductance_ukdale')
COLOR    = '#9467BD'
...
def plot_training_curves(results, save_dir):
    ...
    fig.suptitle('Training Curves — Conductance LTC (UKDALE)', fontsize=12)
    plt.tight_layout()
...
def plot_epoch_metrics(results, save_dir):
    ...
    fig.suptitle('Val Metrics per Epoch — Conductance LTC (UKDALE)', fontsize=12)
    plt.tight_layout()
...
def plot_bar_chart(results, save_dir):
    ...
    fig.suptitle('Final Test Metrics — Conductance LTC (UKDALE)', fontsize=12)
    plt.tight_layout()
...
def print_table(results):
    ...
    for metric, label in [('f1', 'F1'), ('precision', 'Precision'),
        ...
        print(f"\n{'='*70}")
        print(f"  {label} — Conductance LTC (UKDALE)")
        print(f"{'='*70}")
...
def main():
    parser = argparse.ArgumentParser(description='Conductance LTC — UKDALE')
    ...
    if all_done:
        ...
        print('Run those, then: python test_lnn_conductance_ukdale_specific_splits.py --plot')
