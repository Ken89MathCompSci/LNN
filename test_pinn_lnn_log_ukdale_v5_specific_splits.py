"""
Physics-Informed LNN (PINN-LNN) + log1p scaling v5 — UKDALE specific splits
=============================================================================
Three improvements over v4 (avg F1=0.624):

    Fix 7 — FR BCE_ALPHA 1.5 → 1.0
        v4 fridge: P=0.409, R=0.993 — over-predicting ON events.
        Lowering alpha reduces the upweight on positive samples so the model
        is less aggressive about firing for borderline-ON windows.

    Fix 8 — Focal loss (γ=2, α=0.75) for microwave instead of weighted BCE
        v4 MW: P=0.927, R=0.115, F1=0.205 — model is ultra-conservative.
        Root cause: 19:1 class imbalance means easy-negative BCE gradient
        overwhelms the pos_weight signal.  Focal loss multiplicatively
        down-weights easy negatives via (1−p_t)^γ, so the hard positives
        (sparse MW ON events) dominate gradient updates.

    Fix 9 — Two-sided physics reconstruction penalty
        v4 physics loss: 0.000 throughout (one-sided ReLU trivially satisfied).
        Replace ReLU(Σpred_raw − P_agg_raw − ε) with:
            |Σpred_raw − P_agg_raw| / (P_agg_raw + 1)
        Symmetric + always active → physics gradient fires every batch.

v4 fixes (carried forward):
    Fix 6 — Raise MW BCE_LAMBDA 0.1 → 0.2  (keep α=15.0; replaced by focal loss in v5)
    Fix 5 — FR BCE_ALPHA 1.5 (replaced by 1.0 in v5)
...
LAMBDA_PHYS   = 0.01
EPSILON_W     = 50.0   # kept for reference; not used in two-sided loss
WARMUP_EPOCHS = 20
...
# Fix 7: FR alpha 1.5 → 1.0 (reduce over-prediction of fridge ON events)
BCE_ALPHA  = {'dish washer': 2.0, 'fridge': 1.0, 'microwave': 15.0, 'washer dryer': 2.0}

MW_IDX = APPLIANCES.index('microwave')  # 2


# ---------------------------------------------------------------------------
# Focal loss helper  (Fix 8 — replaces weighted BCE for microwave)
# ---------------------------------------------------------------------------

def focal_loss(p, target, alpha=0.75, gamma=2.0):
    """
    Focal loss operating on probabilities (not logits).

    p      : (N,) in [1e-7, 1-1e-7] — model output clamped to valid prob range
    target : (N,) binary {0, 1}
    alpha  : foreground weight (applied to positives)
    gamma  : focusing exponent; gamma=2 is the canonical value

    FL = -α_t · (1 − p_t)^γ · log(p_t)
    where p_t = p if target==1 else 1−p
          α_t = alpha if target==1 else (1−alpha)
    """
    ce    = F.binary_cross_entropy(p, target, reduction='none')
    p_t   = p * target + (1.0 - p) * (1.0 - target)
    a_t   = alpha * target + (1.0 - alpha) * (1.0 - target)
    return (a_t * (1.0 - p_t) ** gamma * ce).mean()


# ---------------------------------------------------------------------------
# Physics Consistency Loss  (Fix 9 — two-sided symmetric penalty)
# ---------------------------------------------------------------------------

class PhysicsConsistencyLoss(nn.Module):
    """
    Two-sided relative reconstruction error:
        |Σ p_hat_i_raw − P_agg_raw| / (P_agg_raw + 1)

    Always active (no dead-zone ReLU), guarantees physics gradient every batch.
    ...
    def __init__(self, x_scaler, y_scalers, appliances, epsilon_w=EPSILON_W):
        super().__init__()
        # epsilon_w kept for API compatibility but not used in two-sided formula
    ...
    def forward(self, x_mid_scaled, pred_scaled):
        ...
        Returns: scalar physics loss (always > 0)
        """
        ...
        # Two-sided: penalise both over- and under-prediction symmetrically
        return ((p_sum - x_raw).abs() / (x_raw + 1.0)).mean()


# ---------------------------------------------------------------------------
# Physics-Informed LNN Model  (unchanged from v4)
# ---------------------------------------------------------------------------
...
def train_pinn_model(data_dict, save_dir,
    ...
    print(f"Scaling: log1p → MinMax  [v5: FR α=1.0, MW focal loss γ=2, two-sided physics]")
    ...
    print("Starting PINN-LNN (log1p v5) training...")
            ...
            if epoch < WARMUP_EPOCHS:
                ...
                for i, app in enumerate(APPLIANCES):
                    if BCE_LAMBDA[app] > 0:
                        ...
                        if i == MW_IDX:
                            # Fix 8: focal loss for microwave (handles 19:1 imbalance)
                            bce_i = focal_loss(pred_i, y_bin, alpha=0.75, gamma=2.0)
                        else:
                            w = torch.where(y_bin == 1,
                                            torch.full_like(y_bin, BCE_ALPHA[app]),
                                            torch.ones_like(y_bin))
                            bce_i = F.binary_cross_entropy(pred_i, y_bin, weight=w)
    ...
    config = {
        ...
        'description': (
            ...
            'v5: FR α=1.0 (fix over-prediction), MW focal loss γ=2 α=0.75, '
            'two-sided physics |Σpred−agg|/(agg+1), two-phase early stopping'
        ),
        'loss': f'MSE + {lambda_phys} * PhysicsConsistency(two-sided) + BCE/Focal',
        'window_size': WIN,
        'model_params': {
            'input_size': 1, 'hidden_size': hidden_size,
            'n_appliances': len(APPLIANCES), 'dt': dt,
        ...
        'train_params': {
            ...
            'mw_focal_gamma': 2.0, 'mw_focal_alpha': 0.75,
        },
        'test_metrics': {
    ...
    with open(os.path.join(save_dir, 'pinn_lnn_log_v5_ukdale_results.json'),
...
def _plot_training(history, test_metrics, save_dir):
    ...
    plt.title('Total Loss (MSE + λ·Phys + BCE/Focal)')
    ...
    plt.title('Physics Consistency Loss (two-sided)')
    plt.xlabel('Epoch'); plt.ylabel('L_phys')
    plt.legend(); plt.grid(True, alpha=0.3)

    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_log_v5_ukdale_loss.png'),
                dpi=150, bbox_inches='tight')
    plt.close()

    fig, axes = plt.subplots(len(APPLIANCES), 2,
                             figsize=(12, 4 * len(APPLIANCES)))
    fig.suptitle('PINN-LNN (log1p v5) UKDALE — Per-Appliance Val Metrics', fontsize=13)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_log_v5_ukdale_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    save_dir  = f"models/pinn_lnn_log_v5_ukdale_{timestamp}"
