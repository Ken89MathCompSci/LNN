"""
Spectral-Stable LNN (S-LNN) — UKDALE specific splits
======================================================
Architecture: SpectralLiquidTimeLayer unrolled over WIN timesteps → final hidden → FC

Standard LNN Euler step:
    dh/dt     = -h/τ + tanh(W_in x + U h)
    h_{t+1}   = clamp(h_t + dt · dh/dt,  -10, 10)

Problem with unconstrained U
    Hidden-state norms grow roughly as  ‖h_t‖ ~ ρ(U)^t
    where ρ(U) is the spectral radius of the recurrent matrix.
    If ρ(U) > 1 the trajectory is unstable:
        • hidden states explode
        • oscillations amplify
        • gradients vanish/explode through BPTT
    Physical note: real appliances are dissipative — energy does NOT grow
    exponentially, so ρ(U) > 1 is physically unrealistic.

Spectral clipping (after every optimizer step):
    spec  = σ_max(U)                    ← largest singular value (= spectral norm)
    if spec > SPEC_RADIUS:
        U ← U × (SPEC_RADIUS / spec)   ← project onto spectral ball

    SPEC_RADIUS = 0.99 keeps the system just inside marginal stability:
        ρ < 1  → stable, decaying memory
        ρ = 1  → marginal (long memory, used in echo-state networks)
        ρ > 1  → unstable (forbidden here)

Why σ_max(U) ≠ ρ(U) in general, but is still the right constraint:
    For a non-symmetric matrix ρ(U) ≤ σ_max(U).
    Bounding σ_max suffices to guarantee stability and also controls the
    Lipschitz constant of the recurrent map, which directly bounds how
    far hidden states can move per step — important for gradient flow.
...
Gradient clipping: max_norm=1.0 every step  (complementary to spectral clipping).
...
Diagnostics:
    spec_h  — spectral norm of U recorded once per epoch (post-clip)
              saved in JSON so you can verify the constraint holds throughout training

Usage:
    python test_slnn_ukdale_specific_splits.py --appliance fridge
    python test_slnn_ukdale_specific_splits.py --appliance microwave
    python test_slnn_ukdale_specific_splits.py --appliance "dish washer"
    python test_slnn_ukdale_specific_splits.py --appliance "washer dryer"
    python test_slnn_ukdale_specific_splits.py --plot
"""
...
SPEC_RADIUS   = 0.99   # spectral-norm ceiling for recurrent matrix U
...
SAVE_DIR = os.path.join('results', 'slnn_ukdale')
COLOR    = '#9467BD'   # purple — distinct from basic-LNN green


# ── Spectral-Stable Liquid Time Layer ─────────────────────────────────────────

class SpectralLiquidTimeLayer(nn.Module):
    """
    Liquid Time-Constant layer with spectral-norm-constrained recurrent matrix.

    ODE:  dh/dt = -h/τ + tanh(W_in x + U h)
    Step: h'    = clamp(h + dt · dh, -10, 10)

    U is exposed as nn.Linear (bias=False) so its .weight tensor can be
    projected in-place after each optimizer step without breaking autograd:

        with torch.no_grad():
            W    = model.liquid_layer.U.weight
            spec = torch.linalg.matrix_norm(W, ord=2)   # σ_max(U)
            if spec > SPEC_RADIUS:
                W.mul_(SPEC_RADIUS / spec)

    Compared to the raw nn.Parameter in the base LiquidTimeLayer:
      • identical forward computation
      • identical parameter count (hidden² weights, no bias)
      • adds in-place post-step projection — zero overhead in the forward pass
    """
    ...
    def __init__(self, input_size: int, hidden_size: int, dt: float = 0.1):
        ...
        # Recurrent matrix — Linear with no bias so weight is exactly U
    ...
    def forward(self, x: torch.Tensor, hidden: torch.Tensor | None = None) -> torch.Tensor:
        ...
        return (hidden + self.dt * dh).clamp(-10.0, 10.0)


# ── Spectral-Stable Liquid Network Model ──────────────────────────────────────

class SpectralLiquidNetworkModel(nn.Module):
    """
    Unrolls SpectralLiquidTimeLayer over the full input window, then maps the
    ...
    def __init__(self, input_size: int, hidden_size: int, output_size: int,
                 dt: float = 0.1):
        super().__init__()
        self.liquid_layer = SpectralLiquidTimeLayer(input_size, hidden_size, dt)
...
def _spectral_norm(model: SpectralLiquidNetworkModel) -> float:
    with torch.no_grad():
...
def train_appliance(appliance_name, splits, device, epochs, hidden_size):
    ...
    model = SpectralLiquidNetworkModel(
    ...
    for epoch in range(epochs):
        ...
        for xb, yb in bar:
            ...
            # ── Spectral clipping: project U onto spectral ball ρ ≤ SPEC_RADIUS ──
            with torch.no_grad():
                W    = model.liquid_layer.U.weight
        ...
        spec_h.append(_spectral_norm(model))   # record post-epoch spectral norm
...
def plot_training_curves(results, save_dir):
    ...
    fig.suptitle('Training Curves — S-LNN (UKDALE)', fontsize=12)
    plt.tight_layout()
...
def plot_epoch_metrics(results, save_dir):
    ...
    for row, app in enumerate(APPLIANCES):
        ...
        for col, (key, label) in enumerate(
            ...
            if label == 'σ_max(U)':
                ax.axhline(SPEC_RADIUS, color='red', ls='--', lw=1,
    ...
    fig.suptitle('Val Metrics per Epoch — S-LNN (UKDALE)', fontsize=12)
    plt.tight_layout()
...
def plot_bar_chart(results, save_dir):
    ...
    fig.suptitle('Final Test Metrics — S-LNN (UKDALE)', fontsize=12)
    plt.tight_layout()
...
def _save_json(app, r, hidden, epochs, save_dir):
    ...
    with open(path, 'w') as f:
        json.dump({
            'appliance':     app,
            'dataset':       'UKDALE',
            'architecture':  'SpectralLiquidNetworkModel',
            'hidden_size':   hidden,
...
def main():
    parser = argparse.ArgumentParser(description='S-LNN — UKDALE')
    ...
    print('Architecture: SpectralLiquidTimeLayer (ODE + σ_max(U) ≤ 0.99) → hidden → FC')
    print(f'Spectral constraint: σ_max(U) clipped to {SPEC_RADIUS} after every optimizer step')
    ...
    if all_done:
        ...
        print('Run those, then: python test_slnn_ukdale_specific_splits.py --plot')
