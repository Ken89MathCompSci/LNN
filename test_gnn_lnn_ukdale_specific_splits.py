"""
GNN + LNN on UKDALE with specific splits.
...
class UKDALEMultiDataset(torch.utils.data.Dataset):
...
def train_gnn_lnn(data_dict, window_size=100,
                  ...
                  save_dir='models/gnn_lnn_ukdale_specific'):
    ...
    train_loader = torch.utils.data.DataLoader(
        UKDALEMultiDataset(X_train, y_train), batch_size=32, shuffle=True)
    val_loader = torch.utils.data.DataLoader(
        UKDALEMultiDataset(X_val, y_val), batch_size=32, shuffle=False)
    test_loader = torch.utils.data.DataLoader(
        UKDALEMultiDataset(X_test, y_test), batch_size=32, shuffle=False)
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            counter = 0
            best_path = os.path.join(save_dir, 'gnn_lnn_ukdale_best.pth')
    ...
    plt.suptitle('GNN-LNN — UKDALE', fontsize=14)
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'gnn_lnn_ukdale_metrics.png'), dpi=300, bbox_inches='tight')
    plt.close()
    ...
    with open(os.path.join(save_dir, 'gnn_lnn_ukdale_history.json'), 'w', encoding='utf-8') as f:
...
if __name__ == "__main__":
    print("Testing GNN-LNN on UKDALE dataset with specific splits...")
    ...
    save_dir = f"models/gnn_lnn_ukdale_specific_test_{timestamp}"
