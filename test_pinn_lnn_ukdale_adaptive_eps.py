"""
Physics-Informed LNN (PINN-LNN) for NILM — UKDALE, adaptive ε.

Same architecture as test_pinn_lnn_ukdale_specific_splits.py but with
data-driven adaptive ε instead of a fixed constant:

    residual_n = max(0, P_agg_n - Σ p_i_true_n)
    ε = μ_residual + k · σ_residual

    A smaller k tightens the physics constraint; a larger k loosens it.
    k ≈ 1 is a good starting point for most residential datasets.
...
BCE_LAMBDA = {'dish washer': 0.2, 'fridge': 0.2, 'microwave': 0.3, 'washer dryer': 0.2}
BCE_ALPHA  = {'dish washer': 1.5, 'fridge': 1.5, 'microwave': 3.0, 'washer dryer': 2.5}
...
def train_pinn_model(data_dict, save_dir,
    ...
    with open(os.path.join(save_dir, 'pinn_lnn_ukdale_adaptive_results.json'),
...
def _plot_training(history, test_metrics, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_ukdale_adaptive_loss.png'),
                dpi=150, bbox_inches='tight')
    plt.close()

    fig, axes = plt.subplots(len(APPLIANCES), 2,
                             figsize=(12, 4 * len(APPLIANCES)))
    fig.suptitle('PINN-LNN UKDALE (adaptive ε) — Per-Appliance Val Metrics', fontsize=13)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_ukdale_adaptive_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    save_dir  = f"models/pinn_lnn_ukdale_adaptive_{timestamp}"
