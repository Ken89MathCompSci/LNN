"""
Physics-Informed Liquid Mixture of Experts (PINN-LMoE) for NILM — REDD.

Architecture:
    N liquid experts, each a CfC (Closed-form Continuous-time) cell with a
    different time constant (fast / medium / slow).  A learned router network
    produces soft weights over the experts at every timestep.  The weighted
    sum of expert hidden states is passed to per-appliance linear heads.

    Expert update (CfC closed-form ODE solution):
        decay_i = exp(−dt_i / τ_i)
        f_t     = tanh(LayerNorm(W_in·x_t + W_rec·h))
        h_new   = decay_i · h  +  (1 − decay_i) · f_t

    Router (soft, differentiable):
        w = softmax(W_gate · x_t)   ∈ ℝ^N

    Combined hidden state:
        h_comb = Σ_i  w_i · h_i

    Per-appliance heads:
        ŷ_k = w_k · LayerNorm(h_comb) + b_k

Loss (two-stage):
    Stage 1 (epochs 1–WARMUP): MSE only
    Stage 2 (remaining):       MSE + λ·L_phys + weighted BCE

Why CfC instead of Euler steps?
    The closed-form decay  exp(−dt/τ)  is the exact solution of the linear
    part of the LNN ODE over interval dt.  It avoids the Euler truncation
    error and stays stable even for large dt (slow experts have dt=0.5).

Why three experts?
    NILM is a multi-scale problem:
      Expert 0 (dt=0.05, fast)   — start-up transients, microwave spikes
      Expert 1 (dt=0.10, medium) — running cycles, fridge compressor
      Expert 2 (dt=0.50, slow)   — long-duration appliances (dish washer)
"""
...
N_EXPERTS     = 3
EXPERT_DTS    = [0.05, 0.10, 0.50]   # fast, medium, slow
...
# Best BCE config from REDD tuning (Run 2)
BCE_LAMBDA = {'dish washer': 0.5, 'fridge': 0.3, 'microwave': 0.3, 'washer dryer': 0.0}
BCE_ALPHA  = {'dish washer': 2.0, 'fridge': 2.0, 'microwave': 0.3, 'washer dryer': 1.0}
...
# ---------------------------------------------------------------------------
# Liquid Expert (CfC cell)
# ---------------------------------------------------------------------------

class LiquidExpert(nn.Module):
    """
    Single CfC cell.  Uses the closed-form ODE solution for the decay term:

        decay = exp(−dt / τ)
        h_new = decay · h  +  (1 − decay) · tanh(LayerNorm(W_in·x + W_rec·h))

    Each expert has its own dt, τ, W_in, W_rec — allowing specialisation to
    different timescales of the power signal.
    """
    ...
    def forward(self, x_t, h):
        ...
        decay = torch.exp(-self.dt / tau)                  # (1, H)

        inp = self.input_proj(x_t)
        rec = torch.matmul(h, self.rec_weights)
        f_t = torch.tanh(self.intra_norm(inp + rec))

        # Closed-form ODE: exact solution over interval dt
        h_new = decay * h + (1.0 - decay) * f_t
        return h_new.clamp(-10.0, 10.0)
...
class Router(nn.Module):
    """
    Soft router: maps current input x_t to a probability distribution over
    N experts.  Uses a single linear layer + softmax.

    Soft (rather than top-k hard) routing is required for:
      (a) differentiability through the expert weighting
      (b) smooth hidden-state transitions (no discontinuous expert switches)
    """
    ...
    def forward(self, x_t):
        """Returns (batch, n_experts) soft weights."""
        return F.softmax(self.gate(x_t), dim=-1)


# ---------------------------------------------------------------------------
# Physics-Informed Liquid MoE Model
# ---------------------------------------------------------------------------

class PhysicsInformedLiquidMoEModel(nn.Module):
    """
    N LiquidExperts (CfC cells, different dt) + soft Router + per-appliance heads.

    At each timestep t:
        1. Router computes w = softmax(W_gate · x_t)
        2. Each expert updates its own hidden state: h_i ← Expert_i(x_t, h_i)
        3. Combined state: h_comb = Σ_i  w_i · h_i

    After the full sequence (T timesteps):
        h_final = LayerNorm(h_comb)
        ŷ_k     = Linear_k(h_final)   for k = 1…n_appliances
    """

    def __init__(self, input_size, hidden_size, n_appliances,
                 n_experts=N_EXPERTS, expert_dts=EXPERT_DTS):
        super().__init__()
        self.hidden_size  = hidden_size
        ...
        self.experts = nn.ModuleList([
            LiquidExpert(input_size, hidden_size, dt=expert_dts[i])
            for i in range(n_experts)
        ])

        self.router = Router(input_size, n_experts)
    ...
    def forward(self, x):
        ...
        # Initialise each expert's hidden state to zero
        h_states = [
        ...
        for t in range(seq_len):
            ...
            # Router weights — soft, differentiable
            w = self.router(x_t)                          # (batch, n_experts)

            # Update each expert's hidden state
            h_states = [
                self.experts[i](x_t, h_states[i])
                for i in range(self.n_experts)
            ]

            # Weighted combination of expert hidden states
            # w[:, i:i+1] broadcasts over hidden dim
            h_comb = sum(
                w[:, i:i+1] * h_states[i]
                for i in range(self.n_experts)
            )                                             # (batch, hidden_size)
...
def train_pinn_model(data_dict, save_dir,
                     hidden_size=64, n_experts=N_EXPERTS,
                     expert_dts=EXPERT_DTS,
                     lambda_phys=LAMBDA_PHYS, epsilon_w=EPSILON_W):
    os.makedirs(save_dir, exist_ok=True)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Using device: {device}")
    print(f"λ_phys={lambda_phys}  ε={epsilon_w} W  hidden={hidden_size}  "
          f"n_experts={n_experts}  dts={expert_dts}")
    ...
    model = PhysicsInformedLiquidMoEModel(
    ...
    print("Starting PINN-LMoE training (all appliances simultaneously)...")
    ...
    config = {
        ...
        'model': 'PhysicsInformedLiquidMoEModel',
        'description': (
            f'{n_experts} CfC experts (dt={expert_dts}) + soft router + '
            f'per-appliance heads + L_phys'
        ),
    ...
    with open(os.path.join(save_dir, 'pinn_lmoe_redd_results.json'),
...
def _plot_training(history, test_metrics, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lmoe_redd_loss.png'),
                dpi=150, bbox_inches='tight')
    plt.close()

    fig, axes = plt.subplots(len(APPLIANCES), 2,
                             figsize=(12, 4 * len(APPLIANCES)))
    fig.suptitle('PINN-LMoE REDD — Per-Appliance Val Metrics', fontsize=13)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lmoe_redd_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    save_dir  = f"models/pinn_lmoe_redd_{timestamp}"
