"""
PINN-LISTA-LNN for NILM — UKDALE specific splits.
...
def train_pinn_lista_model(data_dict, save_dir,
    ...
    for epoch in range(EPOCHS):
        ...
        for xb, yb in bar:
            ...
            power, x   = model(xb)                      # (B, n_apps), (B, n_atoms)
    ...
    with open(os.path.join(save_dir, 'pinn_lista_lnn_ukdale_results.json'),
...
def _plot_training(history, test_metrics, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lista_lnn_ukdale_loss.png'),
                dpi=150, bbox_inches='tight')
    plt.close()

    fig, axes = plt.subplots(len(APPLIANCES), 2,
                             figsize=(12, 4 * len(APPLIANCES)))
    fig.suptitle('PINN-LISTA-LNN UKDALE — Per-Appliance Val Metrics', fontsize=13)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lista_lnn_ukdale_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    save_dir  = f"models/pinn_lista_lnn_ukdale_{timestamp}"
