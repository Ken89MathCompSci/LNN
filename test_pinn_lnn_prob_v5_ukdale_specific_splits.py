"""
Probabilistic Physics-Informed LNN (Prob-PINN-LNN) v5 — UKDALE specific splits
================================================================================
Improves over the base prob model (avg test F1=0.673) with five targeted fixes:

Fix 1 — DW/FR BCE_ALPHA reversal
    Base: BCE_ALPHA > 1 upweights positives (boosts recall).
    DW had P=0.113, R=1.000 — always-ON.  Upweighting positives further
    entrenches the false-positive flood.  Setting BCE_ALPHA < 1 flips the
    weight so that OFF samples get more gradient per example.
        BCE_ALPHA = {dish washer: 0.3, fridge: 0.8, microwave: 15.0, washer dryer: 2.0}
    DW: negatives get 1/0.3 ≈ 3× weight vs positives.
    FR: slight negative upweight (0.8).
    MW/WD: unchanged — already F1 > 0.94.

Fix 2 — Two-sided physics loss
    Base: ReLU(Σmu_raw − P_agg − ε) / σ_sum — zero whenever model under-predicts
    (dead gradient for the majority of training).
    New: |Σmu_raw − P_agg| / σ_sum — symmetric, fires every batch.

Fix 3 — Sigma clamp [0.01, 5.0]
    Base: sigma = softplus(log_s) + 1e-6 — unbounded upper.
    Sigma growing large absorbs prediction errors without improving mu.
    New: sigma = softplus(log_s).clamp(0.01, 5.0) + 1e-6.

Fix 4 — log1p scaling
    Add log1p before MinMaxScaler (same pipeline as deterministic v2–v5).
    Compresses the right-skewed power distribution so MinMax spreads small
    loads across the full [0,1] range rather than compressing them near zero.
    Five update sites: X/Y preprocessing, threshold mapping, physics inverse,
    metrics inverse, uncertainty plot sigma conversion.

Fix 5 — Val NLL for early stopping throughout both phases
    Base BCE phase tracked val F1, which is extremely noisy on the Dec 7 val
    day (appliance usage pattern differs markedly from the test day).
    New: both phases track val NLL; counter resets at warmup→BCE transition
    so BCE gets a full PATIENCE window to find its best NLL-minimising state.

Base model results (for reference):
    dish washer   F1=0.202  P=0.113  R=1.000   ← always-ON
    fridge        F1=0.582  P=0.410  R=1.000   ← over-predicts ON
    microwave     F1=0.945  P=0.962  R=0.928   ← excellent
    washer dryer  F1=0.962  P=0.965  R=0.959   ← excellent
    avg           F1=0.673

Architecture (unchanged):
    Input (batch, WIN, 1) — log1p-MinMax scaled mains window
         |
    Shared AdvancedLiquidTimeLayer encoder
         |
    LayerNorm(hidden)
         |
    mu_heads  (n × Linear)    sigma_heads (n × Linear, bias=-2)
         |                           |
    (batch, n) mu        sigma = softplus(·).clamp(0.01,5.0) + 1e-6
"""
...
LAMBDA_PHYS   = 0.01
EPSILON_W     = 50.0   # kept for API compat; not used in two-sided loss
WARMUP_EPOCHS = 20
...
# Fix 1: alpha < 1 upweights the negative (OFF) class for always-ON appliances;
#        alpha > 1 upweights the positive (ON) class for hard-to-detect appliances.
BCE_ALPHA = {'dish washer': 0.3, 'fridge': 0.8, 'microwave': 15.0, 'washer dryer': 2.0}


# ---------------------------------------------------------------------------
# Probabilistic Physics Consistency Loss  (Fix 2 + Fix 4)
# ---------------------------------------------------------------------------

class ProbabilisticPhysicsLoss(nn.Module):
    """
    Two-sided uncertainty-normalised physics reconstruction penalty (Fix 2):
        |Σ mu_i_raw − P_agg_raw| / sqrt(Σ sigma_i_log^2)

    Symmetric (no dead ReLU zone) and always produces a non-zero gradient.

    Fix 4: inverse pipeline is log1p-aware:
        scaled → MinMax⁻¹ → log-space → expm1 → raw Watts
    """

    def __init__(self, x_scaler, y_scalers, appliances, epsilon_w=EPSILON_W):
        super().__init__()
        # epsilon_w kept for signature compatibility; not used in two-sided formula
    ...
    def forward(self, x_mid_scaled, mu_scaled, sigma_scaled):
        """
        x_mid_scaled : (batch,)        — log1p-MinMax scaled mains midpoint
        mu_scaled    : (batch, n_apps) — predicted means in scaled space
        sigma_scaled : (batch, n_apps) — predicted stds  in scaled space
        """
        # MinMax⁻¹ → log space
        x_log  = x_mid_scaled * self.x_range + self.x_min          # (batch,)
        mu_log = mu_scaled    * self.y_ranges + self.y_mins         # (batch, n_apps)
        # sigma stays in log-space (consistent unit for z-score denominator)
        sigma_log = sigma_scaled * self.y_ranges                    # (batch, n_apps)

        # expm1 → raw Watts
        x_raw  = torch.expm1(x_log.clamp(min=0.0))                 # (batch,)
        mu_raw = torch.expm1(mu_log.clamp(min=0.0))                 # (batch, n_apps)

        mu_sum    = mu_raw.sum(dim=1)                               # (batch,)
        sigma_sum = torch.sqrt((sigma_log ** 2).sum(dim=1) + 1e-8) # (batch,)

        # Fix 2: two-sided symmetric penalty
        return ((mu_sum - x_raw).abs() / (sigma_sum + 1e-8)).mean()


# ---------------------------------------------------------------------------
# Model  (Fix 3: sigma clamp)
# ---------------------------------------------------------------------------
...
class ProbPINNLiquidNetworkModel(nn.Module):
    ...
    Fix 3: sigma = softplus(log_s).clamp(0.01, 5.0) + 1e-6
        Upper clamp prevents sigma from growing to absorb errors;
        lower clamp prevents NLL spikes from collapsed sigma.
    """
    ...
    def forward(self, x):
        ...
        # Fix 3: clamp sigma to prevent collapse (lower) and error absorption (upper)
...
# ---------------------------------------------------------------------------
# Metrics  (Fix 4: expm1 inverse)
# ---------------------------------------------------------------------------

def compute_per_appliance_metrics(y_true, y_mu, y_scalers):
    """
    y_true, y_mu : (N, n_appliances) — log1p-MinMax scaled
    Inverse: MinMax⁻¹ → expm1 → raw Watts → NILM metrics
    """
    metrics = {}
    for i, app in enumerate(APPLIANCES):
        log_true = y_scalers[i].inverse_transform(y_true[:, i:i+1])
        log_pred = y_scalers[i].inverse_transform(y_mu[:,   i:i+1])
        raw_true = np.expm1(log_true.clip(min=0)).flatten()
        raw_pred = np.expm1(log_pred.clip(min=0)).flatten()
...
def train_pinn_prob_model(data_dict, save_dir,
    ...
    print(f"Model: Prob-PINN-LNN v5  λ_phys={lambda_phys}  hidden={hidden_size}  dt={dt}")
    print(f"Scaling: log1p → MinMax  [v5: alpha flip DW/FR, 2-sided phys, sigma clamp, NLL ES]")
    ...
    # Fix 4: log1p before MinMaxScaler
    X_tr = np.log1p(X_tr);  X_va = np.log1p(X_va);  X_te = np.log1p(X_te)
    Y_tr = np.log1p(Y_tr);  Y_va = np.log1p(Y_va);  Y_te = np.log1p(Y_te)
    ...
    # Fix 4: apply log1p to the raw-Watt threshold before MinMax linear inversion
    thresholds_scaled = [
    ...
    for i, app in enumerate(APPLIANCES):
        print(f"  threshold {app}: {THRESHOLDS[app]:.1f} W "
              f"→ log1p={np.log1p(THRESHOLDS[app]):.3f} "
              f"→ scaled={thresholds_scaled[i]:.4f}")
    ...
    print("Starting Prob-PINN-LNN v5 training...")
    ...
    for epoch in range(EPOCHS):
        ...
        for xb, yb in progress_bar:
            ...
            if epoch < WARMUP_EPOCHS:
                ...
                # Fix 1: weight < 1 for alpha-flipped appliances upweights OFF samples
        ...
        # Fix 5: val NLL drives early stopping in both phases.
        # Counter resets at warmup→BCE transition so BCE gets a full PATIENCE
        # window to find its best state independent of the warmup plateau.
        if epoch + 1 == WARMUP_EPOCHS:
            bce_phase    = True
            counter      = 0
            best_val_nll = float('inf')  # fresh best for BCE phase
            print("  [switching to BCE phase — tracking val NLL for early stopping]")
    ...
    config = {
        'dataset':  'UKDALE',
        'model':    'ProbPINNLiquidNetworkModel',
        'scaling':  'log1p → MinMaxScaler',
        'description': (
            'heteroscedastic Gaussian PINN-LNN v5; '
            'GaussianNLL + two-sided probabilistic physics loss + BCE; '
            'Fix1: DW alpha=0.3/FR alpha=0.8 (upweight OFF); '
            'Fix2: |Σmu-agg|/σ two-sided physics; '
            'Fix3: sigma clamp [0.01,5.0]; '
            'Fix4: log1p scaling; '
            'Fix5: val NLL early stopping both phases'
        ),
        'loss': f'GaussianNLL + {lambda_phys} * |Σmu-agg|/σ (two-sided) + BCE',
        'window_size': WIN,
    ...
    with open(os.path.join(save_dir, 'pinn_lnn_prob_v5_ukdale_results.json'),
...
def _plot_training(history, test_metrics, save_dir):
    ...
    plt.title('Total Loss (NLL + λ·Phys + BCE)')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_prob_v5_ukdale_loss.png'),
                dpi=150, bbox_inches='tight')
    plt.close()

    fig, axes = plt.subplots(len(APPLIANCES), 2,
                             figsize=(12, 4 * len(APPLIANCES)))
    fig.suptitle('Prob-PINN-LNN v5 UKDALE — Per-Appliance Val Metrics', fontsize=13)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_prob_v5_ukdale_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
def _plot_uncertainty(y_true, y_mu, y_sigma, y_scalers, save_dir):
    ...
    fig, axes = plt.subplots(len(APPLIANCES), 2,
                             figsize=(12, 4 * len(APPLIANCES)))
    fig.suptitle('Prob-PINN-LNN v5 UKDALE — Test Uncertainty Analysis', fontsize=13)

    for row, app in enumerate(APPLIANCES):
        i = APPLIANCES.index(app)

        log_true = y_scalers[i].inverse_transform(y_true[:, i:i+1]).clip(min=0)
        log_mu   = y_scalers[i].inverse_transform(y_mu[:,   i:i+1]).clip(min=0)
        raw_true = np.expm1(log_true).flatten()
        raw_mu   = np.expm1(log_mu).flatten()
        abs_err  = np.abs(raw_true - raw_mu)

        # sigma: log-space × (1+mu_raw) approximates Watts via delta method
        sigma_log = y_sigma[:, i] * float(y_scalers[i].data_range_[0])
        sigma_raw = sigma_log * (1.0 + raw_mu)
        ...
        ax_hist.set_title(f'{app} — sigma (W, approx)')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_prob_v5_ukdale_uncertainty.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    save_dir  = f"models/pinn_lnn_prob_v5_ukdale_{timestamp}"
