"""
Probabilistic PINN-LNN v2 — UKDALE specific splits
====================================================
Fixes the pathological "always ON" collapse seen in v1.

Root causes of v1 failure
--------------------------
1. BCE applied directly to μ via mu[:, i].clamp(1e-7, 1-1e-7):
   μ is a regression output, NOT a probability.  Forcing continuous power
   predictions into [0,1] creates a strong bias toward always-ON — the model
   discovered it could minimise BCE by always predicting a non-zero value.
   Symptom: all recalls = 1.000, DW precision = 0.057.

2. MinMax scaling → near-constant σ regime.  GaussianNLL learned to hide
   prediction errors behind large σ rather than improving μ accuracy.

3. No structural link between ON/OFF state and power magnitude.  The model
   had no way to express "appliance is OFF → output should be ≈ 0 W".

Fixes in v2
-----------
1. Separate classification head (state_head) per appliance:
       logit = state_head(h)               unconstrained linear
       p_on  = sigmoid(logit)              proper probability ∈ (0, 1)
   Trained with F.binary_cross_entropy_with_logits(pos_weight=α):
       — numerically stable (no manual clamp needed)
       — α upweights ON samples to handle class imbalance

2. Gated power output (key inductive bias for NILM):
       mu_power = mu_head(h)               ON-state power regressor
       mu_final = p_on * mu_power          ≈ 0 when OFF, ≈ mu_power when ON
   GaussianNLL(mu_final, y_true, σ²) jointly drives:
       OFF samples → p_on → 0   (BCE + NLL push mu_final → 0 ≈ y_true)
       ON  samples → p_on → 1, mu_power → y_true  (NLL regression)
   Appliances are mostly OFF: gating enforces this structural prior.

3. log1p + MinMax scaling (from the v3 log series):
       y' = MinMaxScaler(log1p(y))
   Compresses heavy-tailed appliance power distributions; stabilises σ
   learning; reduces the NLL incentive to use σ as an error buffer.
   Inverse: expm1(clip(MinMax⁻¹(y'), 0, ∞))

4. Physics loss with correct log1p inversion for μ and first-order
   Jacobian approximation for σ (chain rule: d(expm1(f)) = (1+y_raw) df):
       mu_raw    = expm1(mu_scaled * y_range + y_min)
       sigma_raw ≈ sigma_scaled * y_range * (1 + mu_raw)

Loss schedule
-------------
    Warmup  (epoch < WARMUP_EPOCHS):
        L = GaussianNLL(mu_final, y, σ²)  +  Σ_i λ_i · BCE(logit_i, y_bin_i)

    F1 phase (epoch ≥ WARMUP_EPOCHS):
        L = GaussianNLL(mu_final, y, σ²)
          + Σ_i λ_i · BCE(logit_i, y_bin_i)
          + λ_phys · ProbPhysics(x_mid, mu_final, σ)

    State BCE is active from epoch 0 (no warmup needed for classification).
    Physics is added after warmup to prevent early spikes destabilising NLL.

Early stopping (two-phase)
    Warmup phase  → best val NLL
    F1 phase      → reset counter, best avg val F1

Monitoring (per epoch)
    val_sigma : mean σ per appliance  — should diversify across appliances
    val_p_on  : mean p_on per appliance — should converge to duty cycle

Architecture
------------
    Input (batch, WIN, 1)
         ↓
    Shared AdvancedLiquidTimeLayer (adaptive τ, input gate)
         ↓
    LayerNorm(hidden)
         ↓
    Per appliance:
        state_head  →  logit  →  sigmoid  →  p_on
        mu_head     →  mu_power
        sigma_head  →  softplus(bias=-2) + 1e-6  →  σ
         ↓
    mu_final = p_on * mu_power     (batch, n_apps)
    Output: (mu_final, σ, logits, p_on)
"""
...
# BCE_LAMBDA: weight of the state BCE term in the total loss (per appliance)
# BCE_ALPHA:  pos_weight for BCEWithLogitsLoss — mirrors v3 log-series values
...
class ProbabilisticPhysicsLoss(nn.Module):
    ...
    Standard (v1): sigma_raw = sigma_scaled * y_range   (ignores curvature)
    This (v2):     sigma_raw = sigma_scaled * y_range * (1 + mu_raw)
                              ← Jacobian of expm1 evaluated at mu_raw

    mu_raw = expm1(mu_scaled * y_range + y_min)   [exact inverse]
    """
    ...
    def forward(self, x_mid_scaled, mu_scaled, sigma_scaled):
        ...
        # First-order σ in raw Watts: chain-rule Jacobian of expm1
...
class GatedProbPINNLiquidNetworkModel(nn.Module):
    ...
    Shared LNN encoder with three per-appliance heads:
        state_head  → logit  (classification, trained with BCEWithLogitsLoss)
        mu_head     → mu_power  (ON-state power regression, unconstrained)
        sigma_head  → σ = softplus(·) + 1e-6  (uncertainty, bias=-2 init)

    Forward output:
        mu_final    = sigmoid(state_logit) * mu_power   (gated power mean)
        sigma                                           (power uncertainty)
    ...
    def __init__(self, input_size, hidden_size, n_appliances, dt=0.1):
        ...
        # Shared encoder (AdvancedLiquidTimeLayer)
    ...
    def forward(self, x):
        ...
        state_logits = torch.cat([head(h) for head in self.state_heads], dim=1)  # (B, n)
        mu_power     = torch.cat([head(h) for head in self.mu_heads],    dim=1)  # (B, n)
        log_s        = torch.cat([head(h) for head in self.sigma_heads],  dim=1) # (B, n)

        p_on     = torch.sigmoid(state_logits)   # (B, n) — proper probability
        sigma    = F.softplus(log_s) + 1e-6       # (B, n) — always positive
        mu_final = p_on * mu_power                # (B, n) — gated output
...
def train_pinn_prob_v2_model(data_dict, save_dir,
                              hidden_size=64, dt=0.1,
                              lambda_phys=LAMBDA_PHYS, epsilon_w=EPSILON_W):
    os.makedirs(save_dir, exist_ok=True)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Using device: {device}")
    print(f"Model: Gated Prob-PINN-LNN v2  λ_phys={lambda_phys}  ε={epsilon_w}W  "
    ...
    # ON/OFF thresholds in log1p-MinMax scaled space
    thresholds_scaled = [
    ...
    history = {
        ...
        'val_metrics': [],
        'val_sigma':   [],   # {app: mean_sigma} per epoch
        'val_p_on':    [],   # {app: mean_p_on}  per epoch
    }
    ...
    print("Starting Gated Prob-PINN-LNN v2 training...")
    ...
    for epoch in range(EPOCHS):
        ...
        for xb, yb in progress_bar:
            ...
            # State BCE — active from epoch 0 (no warmup needed for classification)
    ...
    config = {
        ...
        'model':    'GatedProbPINNLiquidNetworkModel',
        'description': (
            'Prob-PINN-LNN v2: gated output (p_on * mu_power); '
    ...
    with open(os.path.join(save_dir, 'pinn_lnn_prob_v2_ukdale_results.json'),
...
def _plot_training(history, test_metrics, save_dir):
    ...
    fig.suptitle('Gated Prob-PINN-LNN v2 — Training Losses', fontsize=12)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_prob_v2_ukdale_loss.png'),
                dpi=150, bbox_inches='tight')
    plt.close()

    # ── σ and p_on monitoring ──
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 4))
    fig.suptitle('Gated Prob-PINN-LNN v2 — Uncertainty & State Monitoring', fontsize=12)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_prob_v2_ukdale_monitoring.png'),
                dpi=150, bbox_inches='tight')
    plt.close()

    # ── Per-appliance F1 / MAE ──
    fig, axes = plt.subplots(len(APPLIANCES), 2,
                             figsize=(12, 4 * len(APPLIANCES)))
    fig.suptitle('Gated Prob-PINN-LNN v2 UKDALE — Val Metrics', fontsize=13)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_prob_v2_ukdale_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
def _plot_uncertainty(y_true, y_mu, y_sigma, y_p_on, y_scalers, save_dir):
    ...
    fig.suptitle('Gated Prob-PINN-LNN v2 — Test Uncertainty Analysis', fontsize=13)
    ...
    for row, app in enumerate(APPLIANCES):
        ...
        # σ in raw Watts: Jacobian of expm1
        ...
        ax_sig.hist(sigma_raw, bins=50, color='steelblue', alpha=0.75, edgecolor='white')
        ax_sig.axvline(sigma_raw.mean(), color='red', linestyle='--',
                       label=f'mean={sigma_raw.mean():.1f} W')
        ax_sig.set_title(f'{app} — σ distribution (W)')
        ax_sig.set_xlabel('σ (W)'); ax_sig.set_ylabel('Count')
        ax_sig.legend(); ax_sig.grid(True, alpha=0.3)

        ax_scat.scatter(sigma_raw, abs_err, alpha=0.3, s=5, color='steelblue')
        ax_scat.set_title(f'{app} — |error| vs σ')
        ax_scat.set_xlabel('σ (W)'); ax_scat.set_ylabel('|μ_final − y_true| (W)')
        ax_scat.grid(True, alpha=0.3)

        ax_pon.hist(p_on_arr, bins=50, color='coral', alpha=0.75, edgecolor='white')
        ax_pon.axvline(p_on_arr.mean(), color='red', linestyle='--',
                       label=f'mean={p_on_arr.mean():.3f}')
        ax_pon.set_title(f'{app} — p_on distribution')
        ax_pon.set_xlabel('p_on'); ax_pon.set_ylabel('Count')
        ax_pon.legend(); ax_pon.grid(True, alpha=0.3)

    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_prob_v2_ukdale_uncertainty.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    save_dir  = f"models/pinn_lnn_prob_v2_ukdale_{timestamp}"

    data_dict = load_data()

    test_metrics, history, test_sigmas, test_p_ons = train_pinn_prob_v2_model(
        data_dict,
        save_dir    = save_dir,
        hidden_size = 64,
        dt          = 0.1,
