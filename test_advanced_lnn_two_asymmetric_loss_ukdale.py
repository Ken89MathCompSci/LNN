...
def train_on_appliance(data_dict, appliance_name, save_dir,
    ...
    raw_threshold = THRESHOLDS[appliance_name]
    ...
    print(f"Starting Advanced-LNN-Two (Asymmetric Loss) training for {appliance_name}...")
    ...
    for epoch in range(EPOCHS):
        ...
        if avg_va < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir,
                f"advanced_lnn_two_asymmetric_ukdale_{appliance_name.replace(' ', '_')}_best.pth")
            save_model(model,
    ...
    plt.savefig(os.path.join(save_dir,
        f"advanced_lnn_two_asymmetric_ukdale_{appliance_name.replace(' ', '_')}_metrics.png"),
    ...
    with open(os.path.join(save_dir,
            f'advanced_lnn_two_asymmetric_ukdale_{appliance_name.replace(" ", "_")}_history.json'),
...
def run_all(hidden_size=64, num_layers=2, dt=0.1, bce_lambda=0.1):
    data_dict = load_data()
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    base_save_dir = f"models/advanced_lnn_two_asymmetric_loss_ukdale_test_{timestamp}"
    ...
    for appliance_name in APPLIANCES:
        ...
        print(f"\n{'='*60}")
        print(f"Testing Advanced-LNN-Two (Asymmetric Loss) on {appliance_name}")
    ...
    print(f"\nAdvanced-LNN-Two (Asymmetric Loss) UKDALE testing completed.")
