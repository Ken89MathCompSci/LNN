...
def train_gru_lnn_on_appliance(data_dict, appliance_name, window_size=100,
                                ...
                                save_dir='models/gru_lnn_ukdale_specific'):
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir, f"gru_lnn_ukdale_{appliance_name.replace(' ', '_')}_best.pth")
            save_model(model,
    ...
    plt.savefig(os.path.join(save_dir,
        f"gru_lnn_ukdale_{appliance_name.replace(' ', '_')}_metrics.png"),
    ...
    with open(os.path.join(save_dir,
            f'gru_lnn_ukdale_{appliance_name.replace(" ", "_")}_history.json'),
...
def test_gru_lnn_on_all_ukdale_appliances(window_size=100, gru_hidden=64,
    ...
    base_save_dir = f"models/gru_lnn_ukdale_specific_test_{timestamp}"
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path': os.path.join(
                        appliance_dir,
                        f"gru_lnn_ukdale_{appliance_name.replace(' ', '_')}_best.pth"),
    ...
    print(f"\nGRU-LNN UKDALE testing completed. Results saved to {base_save_dir}")
    return all_results


if __name__ == "__main__":
    print("Testing GRU-LNN on UKDALE dataset with specific splits...")
    ...
    results = test_gru_lnn_on_all_ukdale_appliances(
        window_size=100,
        gru_hidden=64,
        num_gru_layers=2,
        hidden_size=64,
    ...
    print(f"\nSummary of GRU-LNN testing on UKDALE dataset:")
