"""
Physics-Informed LNN (PINN-LNN) + log1p scaling v3 — UKDALE specific splits
=============================================================================
Extends v2 with three tuning fixes based on v2 test results:

    Fix 3 — Raise MW BCE_ALPHA 3.0 → 15.0
        At ~2% microwave positive rate (97:3 imbalance), BCE_ALPHA=3 barely
        moves the gradient.  Need ~15× to give minority class a real signal.

    Fix 4 — Enable WD BCE (λ=0.2, α=2.0)
        In v2 the avg-F1 early stopping optimised DW+FR at WD's expense
        (WD F1: 0.878→0.645).  A small BCE signal stabilises WD recall.

    Fix 5 — Lower fridge BCE_ALPHA 2.0 → 1.5
        v2 fridge: Precision=0.41, Recall=0.99 — severely over-predicting ON.
        Reducing the positive-class weight pulls precision up.

v2 fixes (carried forward):
    Fix 1 — Enable BCE for microwave (λ=0.1, α now 15.0)
    Fix 2 — Two-phase early stopping: warmup tracks val_mse, BCE phase tracks avg val F1
...
def train_pinn_model(data_dict, save_dir,
    ...
    print(f"Scaling: log1p → MinMax  [v3: MW α=15, WD BCE enabled, FR α=1.5]")
    ...
    print("Starting PINN-LNN (log1p v3) training...")
    ...
    config = {
        ...
        'description': (
            ...
            'v3: MW α=15.0, WD BCE λ=0.2 α=2.0, FR α=1.5, two-phase early stopping'
        ),
    ...
    with open(os.path.join(save_dir, 'pinn_lnn_log_v3_ukdale_results.json'),
...
def _plot_training(history, test_metrics, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_log_v3_ukdale_loss.png'),
                dpi=150, bbox_inches='tight')
    plt.close()

    fig, axes = plt.subplots(len(APPLIANCES), 2,
                             figsize=(12, 4 * len(APPLIANCES)))
    fig.suptitle('PINN-LNN (log1p v3) UKDALE — Per-Appliance Val Metrics', fontsize=13)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_log_v3_ukdale_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    save_dir  = f"models/pinn_lnn_log_v3_ukdale_{timestamp}"
