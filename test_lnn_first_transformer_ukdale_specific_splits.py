"""
LNN-First Hybrid Transformer — UKDALE specific splits
======================================================
Architecture: LNN (physics frontend) → Transformer Encoder (global context)

The LiquidODECell is unrolled over every raw timestep first, learning
per-channel time constants (tau) that capture transient appliance dynamics.
The resulting hidden-state sequence is then passed to a Transformer Encoder
so self-attention can model long-range, periodic duty cycles.
  ...
  Transformer : 2 × TransformerEncoderLayer(H, nheads=4, ff=4H)
...
Usage (Colab / terminal):
    ...
    !python test_lnn_first_transformer_ukdale_specific_splits.py --appliance fridge
    !python test_lnn_first_transformer_ukdale_specific_splits.py --appliance microwave
    !python test_lnn_first_transformer_ukdale_specific_splits.py --appliance "dish washer"
    !python test_lnn_first_transformer_ukdale_specific_splits.py --appliance "washer dryer"

    # regenerate plots after all four are done:
    !python test_lnn_first_transformer_ukdale_specific_splits.py --plot

    # optional overrides:
    !python test_lnn_first_transformer_ukdale_specific_splits.py --hidden 128 --epochs 60
"""
...
from models import LNNFirstTransformerModel
...
SAVE_DIR = os.path.join('results', 'lnn_first_transformer_ukdale')
COLOR    = '#9467BD'
...
def train_appliance(appliance_name, splits, device, epochs, hidden_size):
    ...
    # Per-channel MinMax normalisation (fit on train only)
    ...
    model = LNNFirstTransformerModel(
...
def plot_training_curves(results, save_dir):
    ...
    fig.suptitle('Training Curves — LNN-First Transformer (UKDALE)', fontsize=12)
    plt.tight_layout()
...
def plot_epoch_metrics(results, save_dir):
    ...
    fig.suptitle('Val Metrics per Epoch — LNN-First Transformer (UKDALE)', fontsize=12)
    plt.tight_layout()
...
def plot_bar_chart(results, save_dir):
    ...
    fig.suptitle('Final Test Metrics — LNN-First Transformer (UKDALE)', fontsize=12)
    plt.tight_layout()
...
def print_table(results):
    ...
    for metric, label in [('f1', 'F1'), ('precision', 'Precision'),
        ...
        print(f"\n{'='*70}")
        print(f"  {label} — LNN-First Transformer (UKDALE)")
        print(f"{'='*70}")
...
def _save_json(app, r, hidden, epochs, save_dir):
    ...
    with open(path, 'w') as f:
        json.dump({
            'appliance':    app,
            'dataset':      'UKDALE',
            'architecture': 'LNN-First → Transformer',
            'hidden_size':  hidden,
            'epochs':       epochs,
...
def main():
    parser = argparse.ArgumentParser(
        description='LNN-First Hybrid Transformer — UKDALE')
    ...
    print('Architecture: Input Proj → LNN (ODE frontend) → Transformer → Pool → FC')
    ...
    if all_done:
        ...
        print(f'Run those, then: python test_lnn_first_transformer_ukdale_specific_splits.py --plot')
