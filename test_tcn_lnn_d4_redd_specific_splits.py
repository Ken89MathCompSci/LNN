...
# 4-block TCN-LNN variant: num_channels=[32,64,128,256], dilation=[1,2,4,8]
# Receptive field = 1 + (3-1)*(1+2+4+8) = 31 steps (vs 15 in 3-block version)
...
def train_tcn_lnn_d4_on_specific_redd_appliance(data_dict, appliance_name, window_size=100,
                                                num_channels=[32, 64, 128, 256], kernel_size=3, dropout=0.2,
                                                hidden_size=64, dt=0.1,
                                                epochs=80, lr=0.001, patience=20,
                                                save_dir='models/tcn_lnn_d4_redd_specific'):
    """
    Train 4-block TCN-LNN (dilation=[1,2,4,8], RF=31) on a specific REDD appliance.
    """
    ...
    print(f"Starting TCN-LNN (4-block, dilation=[1,2,4,8]) training for {appliance_name}...")
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            counter         = 0
            best_model_path = os.path.join(save_dir, f"tcn_lnn_d4_redd_{appliance_name.replace(' ', '_')}_best.pth")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"tcn_lnn_d4_redd_{appliance_name.replace(' ', '_')}_metrics.png"),
    ...
    config = {
        ...
        'model_params': {
            ...
            'kernel_size':  kernel_size,
            'dropout':      dropout,
            'dilation':     [2**i for i in range(len(num_channels))]
        },
    ...
    with open(os.path.join(save_dir, f'tcn_lnn_d4_redd_{appliance_name.replace(" ", "_")}_history.json'), 'w') as f:
        json.dump(config, f, indent=4)

    return model, history, test_metrics


def test_tcn_lnn_d4_on_all_redd_appliances(window_size=100, num_channels=[32, 64, 128, 256], kernel_size=3,
                                           dropout=0.2, hidden_size=64, dt=0.1,
                                           epochs=80, lr=0.001, patience=20):
    """
    Test 4-block TCN-LNN (dilation=[1,2,4,8], RF=31) on all REDD appliances.
    """
    print("Loading REDD data with specific splits...")
    data_dict = load_redd_specific_splits()

    timestamp     = datetime.now().strftime("%Y%m%d_%H%M%S")
    base_save_dir = f"models/tcn_lnn_d4_redd_specific_test_{timestamp}"
    ...
    for appliance_name in appliances:
        print(f"\n{'='*60}")
        print(f"Testing TCN-LNN (4-block) on {appliance_name}")
        print(f"{'='*60}\n")
        ...
        try:
            model, history, test_metrics = train_tcn_lnn_d4_on_specific_redd_appliance(
                data_dict,
            ...
            if model is not None:
                all_results[appliance_name] = {
                    'model_path':    os.path.join(appliance_dir, f"tcn_lnn_d4_redd_{appliance_name.replace(' ', '_')}_best.pth"),
                    'final_metrics': {k: float(v) for k, v in test_metrics.items()}
                }
                print(f"Successfully tested TCN-LNN (4-block) on {appliance_name}")
            else:
                print(f"Failed to test TCN-LNN (4-block) on {appliance_name}")

        except Exception as e:
            print(f"Error testing TCN-LNN (4-block) on {appliance_name}: {str(e)}")
            import traceback
            traceback.print_exc()
            continue
    ...
    summary = {
        'timestamp':    timestamp,
        'model':        'TCN-LNN 4-block dilation=[1,2,4,8] RF=31',
        'dataset_splits': {
    ...
    print(f"\nTCN-LNN (4-block) testing completed on all REDD appliances!")
...
if __name__ == "__main__":
    print("Testing TCN-LNN (4-block, dilation=[1,2,4,8]) on REDD dataset with specific splits...")
    ...
    print("Running comprehensive TCN-LNN (4-block, dilation=[1,2,4,8]) test on all REDD appliances...")
    results = test_tcn_lnn_d4_on_all_redd_appliances(
        window_size=100,
    ...
    print(f"\nSummary of TCN-LNN (4-block) testing on REDD dataset:")
