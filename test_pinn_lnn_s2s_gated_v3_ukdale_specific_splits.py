"""
Physics-Informed LNN (PINN-LNN) for NILM — UKDALE, seq-to-seq with gated heads v3.

Builds on v1 (same architecture and BCE settings) and adds:

  6. Hard-gate evaluation: eval_power = gated_power × (state_logit_recon > 0)
       The state classifier's natural decision boundary (logit = 0, sigmoid = 0.5)
       is used as the ON/OFF gate at inference time.  This replaces thresholding on
       gated_power at 0.0038 (scaled 10 W out of ~2600 W range), which was too low
       for any model output to stay below during OFF periods.

       Procedure:
         - Collect gated_power AND state_logits from every batch during eval.
         - Reconstruct both full sequences via overlap-averaging.
         - Apply gate: effective_pred = gated_power_recon × (state_logit_recon > 0)
         - Inverse-transform effective_pred and compute NILM metrics.

       Training is unchanged from v1; only the evaluation signal changes.
       BCE still trains state_head to push logits negative for OFF periods — now
       that binary signal directly controls the evaluation gate.
"""
...
# All four appliances now enabled
...
def compute_per_appliance_metrics(y_true_recon, y_pred_recon, y_scalers,
    ...
    Args:
        ...
        y_pred_recon:     (T, n_apps) — reconstructed gated_power sequence (scaled)
        y_scalers:        list of MinMaxScaler, one per appliance
        state_logit_recon: (T, n_apps) or None — reconstructed state logits.
                           When provided, applies hard gate:
                               effective_pred = gated_power × (state_logit > 0)
                           This uses the classifier's natural 0.5-sigmoid boundary
                           instead of the 10-W power threshold on a 0–2600-W range.
    Returns:
    ...
    for i, app in enumerate(APPLIANCES):
        ...
        if state_logit_recon is not None:
            hard_gate = (state_logit_recon[:, i] > 0).astype(np.float32)
            gated = (y_pred_recon[:, i] * hard_gate).clip(0, 1).reshape(-1, 1)
        else:
            gated = y_pred_recon[:, i:i+1].clip(0, 1)

        raw_pred = y_scalers[i].inverse_transform(gated).flatten()
...
def train_pinn_model(data_dict, save_dir,
    ...
    for epoch in range(EPOCHS):
        ...
        val_preds, val_trues, val_logits = [], [], []

        with torch.no_grad():
            for xb, yb in va_loader:
                xb, yb = xb.to(device), yb.to(device)
                gated_power, state_logits = model(xb)
                ...
                val_logits.append(state_logits.cpu().numpy())
    ...
    with open(os.path.join(save_dir, 'pinn_lnn_s2s_gated_v3_ukdale_results.json'),
...
def _plot_training(history, test_metrics, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_s2s_gated_v3_ukdale_loss.png'),
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_s2s_gated_v3_ukdale_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    save_dir  = f"models/pinn_lnn_s2s_gated_v3_ukdale_{timestamp}"
