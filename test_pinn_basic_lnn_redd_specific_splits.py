"""
Physics-Informed Basic LNN (PINN-BasicLNN) for NILM — REDD specific splits.

Same structure as test_pinn_basic_lnn_ukdale_specific_splits.py but uses
the REDD dataset.

Architecture:
    Input (batch, WIN, 1)  — scaled mains window
         ↓
    BasicLiquidTimeLayer (fixed tau, no gate, intra-layer LayerNorm)
         ↓
    LayerNorm(hidden)
         ↓
    ┌────┬────┬────┬────┐
...
BCE_LAMBDA = {'dish washer': 0.2, 'fridge': 0.2, 'microwave': 0.3, 'washer dryer': 0.3}
BCE_ALPHA  = {'dish washer': 1.5, 'fridge': 1.5, 'microwave': 0.3, 'washer dryer': 0.3}
...
# ---------------------------------------------------------------------------
# Basic LNN Model (fixed tau, no gate, intra-layer LayerNorm)
# ---------------------------------------------------------------------------
...
def train_pinn_model(data_dict, save_dir,
    ...
    config = {
        'dataset': 'REDD',
        'model': 'PhysicsInformedBasicLiquidNetworkModel',
        'description': 'basic LNN cell (fixed tau, no gate, intra-norm) + per-appliance heads + L_phys',
    ...
    with open(os.path.join(save_dir, 'pinn_basic_lnn_redd_results.json'),
...
def _plot_training(history, test_metrics, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_basic_lnn_redd_loss.png'),
                dpi=150, bbox_inches='tight')
    plt.close()

    fig, axes = plt.subplots(len(APPLIANCES), 2,
                             figsize=(12, 4 * len(APPLIANCES)))
    fig.suptitle('PINN-BasicLNN REDD — Per-Appliance Val Metrics', fontsize=13)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_basic_lnn_redd_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    save_dir  = f"models/pinn_basic_lnn_redd_{timestamp}"
