"""
Physics-Informed Liquid Mixture of Experts (PINN-LMoE) for NILM — UKDALE.

Same architecture as test_pinn_lmoe_redd_specific_splits.py but uses
the UKDALE dataset.
...
# UKDALE: dish washer and fridge are class-imbalanced
...
class Router(nn.Module):
    """Soft router: maps x_t → probability distribution over N experts."""
...
def train_pinn_model(data_dict, save_dir,
    ...
    with open(os.path.join(save_dir, 'pinn_lmoe_ukdale_results.json'),
...
def _plot_training(history, test_metrics, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lmoe_ukdale_loss.png'),
                dpi=150, bbox_inches='tight')
    plt.close()

    fig, axes = plt.subplots(len(APPLIANCES), 2,
                             figsize=(12, 4 * len(APPLIANCES)))
    fig.suptitle('PINN-LMoE UKDALE — Per-Appliance Val Metrics', fontsize=13)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lmoe_ukdale_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    save_dir  = f"models/pinn_lmoe_ukdale_{timestamp}"
