...
def train_on_appliance(data_dict, appliance_name, save_dir,
    ...
    for epoch in range(EPOCHS):
        ...
        if avg_va < best_val_loss:
            ...
            best_model_path = os.path.join(
                save_dir,
                f"advanced_lnn_two_asymmetric_redd_{appliance_name.replace(' ', '_')}_best.pth")
            save_model(model,
    ...
    plt.savefig(os.path.join(save_dir,
        f"advanced_lnn_two_asymmetric_redd_{appliance_name.replace(' ', '_')}_metrics.png"),
    ...
    with open(os.path.join(save_dir,
            f'advanced_lnn_two_asymmetric_redd_{appliance_name.replace(" ", "_")}_history.json'),
...
def run_all(hidden_size=64, num_layers=2, dt=0.1, bce_lambda=0.1):
    ...
    base_save_dir = f"models/advanced_lnn_two_asymmetric_loss_redd_test_{timestamp}"
    ...
    print(f"\nAdvanced-LNN-Two (Asymmetric Loss) REDD testing completed.")
