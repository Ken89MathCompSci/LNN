...
def train_tcn_on_specific_redd_appliance(data_dict, appliance_name, window_size=100,
                                       num_channels=[32, 64, 128], kernel_size=3, dropout=0.2,
                                       epochs=20, lr=0.001, patience=5, save_dir='models/tcn_redd_specific'):
    """
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(save_dir, f"tcn_redd_{appliance_name.replace(' ', '_')}_best.pth")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"tcn_redd_{appliance_name.replace(' ', '_')}_metrics.png"), dpi=300, bbox_inches='tight')
    plt.close()
    plt.savefig(os.path.join(save_dir, f"tcn_redd_{appliance_name.replace(' ', '_')}_training_history.png"))
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"tcn_redd_{appliance_name.replace(' ', '_')}_predictions.png"))
    plt.close()
    ...
    with open(os.path.join(save_dir, f'tcn_redd_{appliance_name.replace(" ", "_")}_history.json'), 'w') as f:
        json.dump(config, f, indent=4)
    
    return model, history, test_metrics

def test_tcn_on_all_redd_appliances(window_size=100, num_channels=[32, 64, 128], kernel_size=3,
                                  dropout=0.2, epochs=20, lr=0.001, patience=10):
    """
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    base_save_dir = f"models/tcn_redd_specific_test_{timestamp}"
    ...
    for appliance_name in appliances:
        ...
        try:
            ...
            model, history, test_metrics = train_tcn_on_specific_redd_appliance(
                data_dict,
            ...
            if model is not None:
                ...
                all_results[appliance_name] = {
                    'model_path': os.path.join(appliance_dir, f"tcn_redd_{appliance_name.replace(' ', '_')}_best.pth"),
                    'final_metrics': {k: float(v) for k, v in test_metrics.items()}
                }
                print(f"✅ Successfully tested TCN on {appliance_name}")
            else:
                print(f"❌ Failed to test TCN on {appliance_name}")
        
        except Exception as e:
            print(f"Error testing TCN on {appliance_name}: {str(e)}")
            import traceback
            traceback.print_exc()
            continue
    ...
    print(f"\n🎉 TCN testing completed on all REDD appliances with specific splits!")
    print(f"Results saved to {base_save_dir}")
    
    return all_results

if __name__ == "__main__":
    print("Testing TCN algorithm on REDD dataset with specific splits...")
    ...
    print("Running comprehensive TCN test on all REDD appliances with specific splits...")
    results = test_tcn_on_all_redd_appliances(
        window_size=100,
    ...
    # Print summary
    print(f"\n📊 Summary of TCN testing on REDD dataset with specific splits:")
