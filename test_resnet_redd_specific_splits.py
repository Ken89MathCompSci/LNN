...
def train_resnet_on_specific_redd_appliance(data_dict, appliance_name, window_size=100,
                                       base_width=32, layers=[2, 2, 2], epochs=20, lr=0.001,
                                       patience=5, save_dir='models/resnet_redd_specific'):
    """
    Train ResNet model on a specific REDD appliance with the exact splits you specified
    ...
    Args:
        ...
        base_width: Base width for ResNet (number of channels in first layer)
        layers: List specifying number of blocks in each layer [2, 2, 2]
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(save_dir, f"resnet_redd_{appliance_name.replace(' ', '_')}_best.pth")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"resnet_redd_{appliance_name.replace(' ', '_')}_metrics.png"), dpi=300, bbox_inches='tight')
    plt.close()
    ...
    for i in range(num_examples):
        ...
        if i == 0:
            plt.ylabel('Power')
    plt.suptitle(f'ResNet Predictions vs Actual - {appliance_name}')
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f"resnet_redd_{appliance_name.replace(' ', '_')}_predictions.png"))
    plt.close()
    ...
    with open(os.path.join(save_dir, f'resnet_redd_{appliance_name.replace(" ", "_")}_history.json'), 'w') as f:
        json.dump(config, f, indent=4)
    
    return model, history, test_metrics

def test_resnet_on_all_redd_appliances(window_size=100, base_width=32, layers=[2, 2, 2],
                                     epochs=20, lr=0.001, patience=10):
    """
    Test ResNet model on all specified REDD appliances with the exact splits you mentioned
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    base_save_dir = f"models/resnet_redd_specific_test_{timestamp}"
    
    # Dictionary to store results
    all_results = {}
    
    # Test ResNet model on each specified appliance
    appliances = ['dish washer', 'fridge', 'microwave', 'washer dryer']
    ...
    for appliance_name in appliances:
        ...
        try:
            ...
            model, history, test_metrics = train_resnet_on_specific_redd_appliance(
                data_dict,
            ...
            if model is not None:
                ...
                all_results[appliance_name] = {
                    'model_path': os.path.join(appliance_dir, f"resnet_redd_{appliance_name.replace(' ', '_')}_best.pth"),
                    'final_metrics': {k: float(v) for k, v in test_metrics.items()}
                }
                print(f"✅ Successfully tested ResNet on {appliance_name}")
            else:
                print(f"❌ Failed to test ResNet on {appliance_name}")
        
        except Exception as e:
            print(f"Error testing ResNet on {appliance_name}: {str(e)}")
            import traceback
            traceback.print_exc()
            continue
    ...
    print(f"\n🎉 ResNet testing completed on all REDD appliances with specific splits!")
    print(f"Results saved to {base_save_dir}")
    
    return all_results

if __name__ == "__main__":
    print("Testing ResNet algorithm on REDD dataset with specific splits...")
    ...
    print("Running comprehensive ResNet test on all REDD appliances with specific splits...")
    results = test_resnet_on_all_redd_appliances(
        window_size=100,
        base_width=32,
        layers=[2, 2, 2],
        epochs=20,
    ...
    # Print summary
    print(f"\n📊 Summary of ResNet testing on REDD dataset with specific splits:")
