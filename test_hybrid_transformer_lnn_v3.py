...
Usage:
    ...
    !python test_hybrid_transformer_lnn.py --appliance fridge
    !python test_hybrid_transformer_lnn.py --appliance microwave
    !python test_hybrid_transformer_lnn.py --appliance "dish washer"
    !python test_hybrid_transformer_lnn.py --appliance "washer dryer"

    # After all four done, generate plots:
    !python test_hybrid_transformer_lnn.py --plot
...
# ── KL divergence loss (from MATNilm loss_func.py) ────────────────────────────
# Measures distributional alignment between predicted and true power distributions.
# Applied per-batch on top of MSE: total_loss = MSE + KL_WEIGHT * KL

def _kl_loss(y_pred, y_true, tau=0.1):
    sp = F.softmax(y_pred / tau, dim=0)
    st = F.softmax(y_true / tau, dim=0)
    return F.kl_div(sp.log(), st, reduction='batchmean')
...
def train_appliance(appliance_name, splits, device, epochs, hidden_size):
    ...
    for epoch in range(epochs):
        ...
        for xb, yb in tqdm(tr_loader,
            ...
            if KL_WEIGHT > 0:
                loss = loss + KL_WEIGHT * _kl_loss(out, yb)
            loss.backward()
...
def _save_appliance_json(app, r, hidden, epochs):
    """Save per-appliance result to its own JSON (persists across Colab sessions)."""
...
def main():
    ...
    if args.plot:
        ...
        if not results:
            ...
            sys.exit(1)
        # Wrap so plot helpers find r['metrics'] etc.
        print('\nGenerating plots...')
    ...
    if all_done:
        ...
        print('Run those, then: !python test_hybrid_transformer_lnn.py --plot')
