"""
Physics-Informed LNN (PINN-LNN) + log1p scaling v4 — UKDALE specific splits
=============================================================================
Extends v3 with one tuning fix based on v3 test results:

    Fix 6 — Raise MW BCE_LAMBDA 0.1 → 0.2  (keep α=15.0)
        v3 microwave: P=0.930, R=0.140, F1=0.243
        Recall=0.140 is too low — the model only fires on the most obvious ON
        events.  Doubling the BCE weight pushes more gradient toward recall
        while α=15 keeps the positive-class weighting high enough to prevent
        precision from collapsing.  Expected: P ~0.7–0.8, R ~0.3–0.4.

v3 fixes (carried forward):
    Fix 3 — MW BCE_ALPHA 15.0
    Fix 4 — WD BCE λ=0.2, α=2.0
    Fix 5 — FR BCE_ALPHA 1.5
...
BCE_LAMBDA = {'dish washer': 0.5, 'fridge': 0.3, 'microwave': 0.2, 'washer dryer': 0.2}
...
def train_pinn_model(data_dict, save_dir,
    ...
    print(f"Scaling: log1p → MinMax  [v4: MW λ=0.2 α=15, WD BCE, FR α=1.5]")
    ...
    print("Starting PINN-LNN (log1p v4) training...")
    ...
    config = {
        ...
        'description': (
            ...
            'v4: MW λ=0.2 α=15.0, WD BCE λ=0.2 α=2.0, FR α=1.5, two-phase early stopping'
        ),
    ...
    with open(os.path.join(save_dir, 'pinn_lnn_log_v4_ukdale_results.json'),
...
def _plot_training(history, test_metrics, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_log_v4_ukdale_loss.png'),
                dpi=150, bbox_inches='tight')
    plt.close()

    fig, axes = plt.subplots(len(APPLIANCES), 2,
                             figsize=(12, 4 * len(APPLIANCES)))
    fig.suptitle('PINN-LNN (log1p v4) UKDALE — Per-Appliance Val Metrics', fontsize=13)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_log_v4_ukdale_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    save_dir  = f"models/pinn_lnn_log_v4_ukdale_{timestamp}"
