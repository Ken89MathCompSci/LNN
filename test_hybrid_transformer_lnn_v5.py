"""
Hybrid Transformer-LNN v5 — Bidirectional LNN + Graduated CNN
==============================================================
Builds on v4 with two structural improvements drawn from existing models:
...
Architecture:
  ...
  → BiDir LNN (fwd + bwd) → Dropout → FC(512→1)

Usage:
    # Run one appliance per session:
    !python test_hybrid_transformer_lnn_v5.py --appliance fridge
    !python test_hybrid_transformer_lnn_v5.py --appliance microwave
    !python test_hybrid_transformer_lnn_v5.py --appliance "dish washer"
    !python test_hybrid_transformer_lnn_v5.py --appliance "washer dryer"

    # After all four done, generate plots:
    !python test_hybrid_transformer_lnn_v5.py --plot

    # Or run all at once (if session allows):
    !python test_hybrid_transformer_lnn_v5.py --hidden 128
"""
...
from models import HybridTransformerLNNv5Model
...
def plot_training_curves(results, save_dir):
    ...
    fig.suptitle('Training Curves — Hybrid Transformer-LNN v5 (80 epochs)', fontsize=12)
    plt.tight_layout()
...
def plot_epoch_metrics(results, save_dir):
    ...
    for row, app in enumerate(APPLIANCES):
        ...
        for col, (hist_key, mk_label) in enumerate(metric_info):
            ...
            ax.grid(True, alpha=0.3)
    fig.suptitle('Val Metrics per Epoch — Hybrid Transformer-LNN v5 (80 epochs)', fontsize=12)
    plt.tight_layout()
...
def plot_bar_chart(results, save_dir):
    ...
    fig.suptitle('Final Metrics — Hybrid Transformer-LNN v5 (80 epochs)', fontsize=12)
    plt.tight_layout()
...
def print_table(results):
    ...
    for metric, label in [('f1', 'F1'), ('mae', 'MAE'), ('sae', 'SAE')]:
        print(f"\n{'='*75}")
        print(f"  {label} — Hybrid Transformer-LNN v5")
        print(f"{'='*75}")
...
def main():
    parser = argparse.ArgumentParser(
        description='Hybrid Transformer-LNN v5 — BiDir LNN + Graduated CNN + Adam')
    ...
    print('Architecture: Learnable Encoding → Graduated CNN (H/4→H/2→H) '
          '→ Transformer → BiDir LNN → Dropout → FC(H*2→1)')

    splits = load_data()
    apps_to_run = [args.appliance] if args.appliance else APPLIANCES

    print('\n' + '=' * 70)
    print('  Hybrid Transformer-LNN v5 — Training')
    print('=' * 70)
    ...
    if all_done:
        ...
        print('Run those, then: !python test_hybrid_transformer_lnn_v5.py --plot')
