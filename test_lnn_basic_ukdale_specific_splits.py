"""
Basic LNN (LiquidNetworkModel) — UKDALE specific splits
========================================================
...
Usage:
    !python test_lnn_basic_ukdale_specific_splits.py --appliance fridge
    !python test_lnn_basic_ukdale_specific_splits.py --appliance microwave
    !python test_lnn_basic_ukdale_specific_splits.py --appliance "dish washer"
    !python test_lnn_basic_ukdale_specific_splits.py --appliance "washer dryer"
    !python test_lnn_basic_ukdale_specific_splits.py --plot
"""
...
SAVE_DIR = os.path.join('results', 'lnn_basic_ukdale')
COLOR    = '#2CA02C'
...
def train_appliance(appliance_name, splits, device, epochs, hidden_size):
    ...
    # CosineAnnealingLR steps only after warmup; LR stays flat during MSE-only phase
...
def plot_training_curves(results, save_dir):
    ...
    fig.suptitle('Training Curves — Basic LNN (UKDALE)', fontsize=12)
    plt.tight_layout()
...
def plot_epoch_metrics(results, save_dir):
    ...
    fig.suptitle('Val Metrics per Epoch — Basic LNN (UKDALE)', fontsize=12)
    plt.tight_layout()
...
def plot_bar_chart(results, save_dir):
    ...
    fig.suptitle('Final Test Metrics — Basic LNN (UKDALE)', fontsize=12)
    plt.tight_layout()
...
def print_table(results):
    ...
    for metric, label in [('f1', 'F1'), ('precision', 'Precision'),
        ...
        print(f"\n{'='*70}")
        print(f"  {label} — Basic LNN (UKDALE)")
        print(f"{'='*70}")
...
def main():
    parser = argparse.ArgumentParser(description='Basic LNN — UKDALE')
    ...
    if all_done:
        ...
        print('Run those, then: python test_lnn_basic_ukdale_specific_splits.py --plot')
