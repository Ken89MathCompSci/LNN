"""
Hybrid Transformer-SSM — Bidirectional Selective SSM + Graduated CNN
=====================================================================
Replaces the LiquidODECell (Euler-integrated ODE) from v5 with a
SelectiveSSMCell (S6-inspired), adapting the core idea from Bi-Mamba-UNet:

  1. Graduated CNN channels  — 64 → 128 → 256 (hidden//4 → hidden//2 → hidden)
  2. Bidirectional SSM       — forward + backward SelectiveSSMCell after the
                               Transformer encoder; pool over ALL hidden states.
  3. Selective mechanism     — B, C, Δ are all functions of the input x_t:
                                 dt    = Softplus(W_Δ · x)   step size (≥ 0)
                                 B     = sigmoid(W_B · x)    write gate
                                 C     = sigmoid(W_C · x)    read  gate
                                 Ā     = exp(dt · A)          decay ∈ (0,1)
                                 h_new = Ā·h + (1−Ā)·B·x_proj
                               Large dt (spike) → state updates aggressively.
                               Small dt (quiet) → old state is preserved.
  4. Dropout before FC       — 0.1 dropout on the concatenated SSM output.
  5. Adam optimiser.
  6. Weighted MSE loss       — active samples (yb > 0 in normalised space) are
                               weighted 2× to counter class imbalance.
  7. Appliance-specific pool — microwave uses max-pool (bursty spikes);
                               all others use mean-pool.

Architecture:
  Learnable Encoding → Graduated CNN (64→128→256) → Transformer Encoder
  → BiDir SelectiveSSM (fwd + bwd) → Dropout → FC(512→1)

Usage:
    # Run one appliance per session:
    !python test_hybrid_transformer_ssm.py --appliance fridge
    !python test_hybrid_transformer_ssm.py --appliance microwave
    !python test_hybrid_transformer_ssm.py --appliance "dish washer"
    !python test_hybrid_transformer_ssm.py --appliance "washer dryer"

    # After all four done, generate plots:
    !python test_hybrid_transformer_ssm.py --plot

    # Or run all at once (if session allows):
    !python test_hybrid_transformer_ssm.py --hidden 128
"""
...
from models import HybridTransformerSSMModel
...
SAVE_DIR = os.path.join('results', 'hybrid_transformer_ssm')
COLOR    = '#9467BD'  # purple
...
def train_appliance(appliance_name, splits, device, epochs, hidden_size):
    ...
    ssm_pool = 'max' if appliance_name == 'microwave' else 'mean'
    model = HybridTransformerSSMModel(
        input_size=1, hidden_size=hidden_size, output_size=1,
        num_conv_layers=3, num_encoder_layers=2, num_heads=4,
        dropout=0.1, pool=ssm_pool,
    ).to(device)
...
def plot_training_curves(results, save_dir):
    ...
    fig.suptitle('Training Curves — Hybrid Transformer-SSM (80 epochs)', fontsize=12)
    plt.tight_layout()
...
def plot_epoch_metrics(results, save_dir):
    ...
    for row, app in enumerate(APPLIANCES):
        ...
        for col, (hist_key, mk_label) in enumerate(metric_info):
            ...
            ax.grid(True, alpha=0.3)
    fig.suptitle('Val Metrics per Epoch — Hybrid Transformer-SSM (80 epochs)', fontsize=12)
    plt.tight_layout()
...
def plot_bar_chart(results, save_dir):
    ...
    fig.suptitle('Final Metrics — Hybrid Transformer-SSM (80 epochs)', fontsize=12)
    plt.tight_layout()
...
def print_table(results):
    ...
    for metric, label in [('f1', 'F1'), ('mae', 'MAE'), ('sae', 'SAE')]:
        print(f"\n{'='*75}")
        print(f"  {label} — Hybrid Transformer-SSM")
        print(f"{'='*75}")
...
def main():
    parser = argparse.ArgumentParser(
        description='Hybrid Transformer-SSM — BiDir SelectiveSSM + Graduated CNN')
    ...
    print('Architecture: Learnable Encoding → Graduated CNN (H/4→H/2→H) '
          '→ Transformer → BiDir SelectiveSSM → Dropout → FC(H*2→1)')

    splits = load_data()
    apps_to_run = [args.appliance] if args.appliance else APPLIANCES

    print('\n' + '=' * 70)
    print('  Hybrid Transformer-SSM — Training')
    print('=' * 70)
    ...
    if all_done:
        ...
        print('Run those, then: !python test_hybrid_transformer_ssm.py --plot')
