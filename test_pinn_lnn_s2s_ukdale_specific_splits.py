"""
Physics-Informed LNN (PINN-LNN) for NILM — UKDALE, sequence-to-sequence.

Extends test_pinn_lnn_ukdale_specific_splits.py with seq-to-seq prediction:
  - Model outputs power at EVERY timestep in the window (batch, WIN, n_apps)
...
Architecture:
    ...
    Shared AdvancedLiquidTimeLayer — hidden state collected at EVERY step
         ↓
    LayerNorm(batch, WIN, hidden)
         ↓
    ┌────┬────┬────┬────┐
    │ DW │ FR │ MW │ WD │  — one Linear head per appliance, applied to each step
    └────┴────┴────┴────┘
    output: (batch, WIN, 4)
...
class PhysicsConsistencyLoss(nn.Module):
    ...
    def forward(self, x_scaled, pred_scaled):
        ...
        Args:
            ...
            pred_scaled: (batch, WIN, n_apps) — scaled appliance predictions
        Returns:
            scalar physics loss
        """
...
# ---------------------------------------------------------------------------
# Seq-to-Seq Physics-Informed LNN
# ---------------------------------------------------------------------------

class PhysicsInformedLiquidNetworkModel(nn.Module):
    """
    Shared AdvancedLiquidTimeLayer — hidden state collected at every step.
    Each per-appliance head is applied to all WIN hidden states in parallel.
    Output: (batch, WIN, n_appliances).
    """
...
def train_pinn_model(data_dict, save_dir,
    ...
    print("Starting PINN-LNN seq-to-seq training (all appliances simultaneously)...")
    ...
    with open(os.path.join(save_dir, 'pinn_lnn_s2s_ukdale_results.json'),
...
def _plot_training(history, test_metrics, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_s2s_ukdale_loss.png'),
                dpi=150, bbox_inches='tight')
    plt.close()

    fig, axes = plt.subplots(len(APPLIANCES), 2,
                             figsize=(12, 4 * len(APPLIANCES)))
    fig.suptitle('PINN-LNN S2S UKDALE — Per-Appliance Val Metrics', fontsize=13)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'pinn_lnn_s2s_ukdale_per_appliance.png'),
                dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    save_dir  = f"models/pinn_lnn_s2s_ukdale_{timestamp}"
